...
//				success: function(data) {
					...
					portal_ZGBM.topProgress.progress();
...
//			});
			...
			portal_ZGBM.topProgress.xsjdPosition($("#agencyProjectPaymentUsed").width() + $("#agencyProjectPaymentUsing").width());
			// 基本支出
...
//			}
		...
		show : function(agency_code, autoExpand) {
			if(!this.inited){
				payProgressViewModel = {
					...
					onRowFocus1 : function(obj) {
						if(portal_ZGBM.payProgress.datalist.length == 0){
							return;
						}
						var row = portal_ZGBM.payProgress.datalist[obj.rowIndex];
						...
						portal_ZGBM.payProgressDetail.show(agency_code, agencyexp_code);
					},
					dataSourceInitRows : null,
					
					onSortFun1 : function(field, sortType){
						if(portal_ZGBM.payProgressDetail.dataSourceInitRows == null){
							portal_ZGBM.payProgressDetail.dataSourceInitRows = this.dataSourceObj.rows;
						}
						if(!field){
							this.dataSourceObj.rows = portal_ZGBM.payProgressDetail.dataSourceInitRows;
							return;
						}
			...
			$.ajax({
				...
				success : function(data) {
					...
					portal_ZGBM.payProgress.datalist = datas;
					payProgressViewModel.dataTable.removeAllRows();
					if(portal_ZGBM.payProgressDetail){
						portal_ZGBM.payProgressDetail.removeAllRows();
					}
					if(autoExpand){
					...
					//第一次打开时，构建部门总体进度情况。
					if(!portal_ZGBM.payProgress.first){
						return;
					}
					portal_ZGBM.payProgress.first = false;
	...
	payProgressDetail : {
		...
		show : function(agency_code, agencyexp_code) {
			if(!this.inited){
				this.payProgressDetailViewModel = {
					...
					onDblClickFun1 : function(obj) {
						if(portal_ZGBM.payProgressDetail.datalist.length == 0){
							return;
						}
						var row = portal_ZGBM.payProgressDetail.datalist[obj.rowIndex];
						var title = "执行情况";
			...
			if(cacheData != null){
				portal_ZGBM.payProgressDetail.datalist = cacheData;
			...
			$.ajax({
				...
				success : function(data) {
					...
					portal_ZGBM.payProgressDetail.datalist = datas;
					portal_ZGBM.payProgressDetail.payProgressDetailViewModel.dataTable.removeAllRows();
					portal_ZGBM.payProgressDetail.payProgressDetailViewModel.dataTable.setSimpleData(datas, {
						ubSelect : true
					});
					portal_ZGBM.payProgressDetail.dataCache[agency_code + agencyexp_code] = datas;
				}
			});
	...
	rightAgencyTree : {
		bf : function() {
			...
			$("#confirm").on("click", function() {
				portal_ZGBM.rightAgencyTree.confirm();
			});
			$("#cancel").on("click", function() {
				portal_ZGBM.rightAgencyTree.cancel();
			});
		},
		show : function() {
			var all_options = {
		...
		confirm : function(){
			...
			portal_ZGBM.payProgress.show(result, true);
			$("#rightAgency").click();
		},
		cancel : function(){
			$("#rightAgency").click();
...
$(function() {
    ...
    // 部门总体进度
    // portal_ZGBM.topProgress.show();
    // $("#agencyTopProgressRefresh").click(function() {
    //     var selectAgency = portal_ZGBM.rightAgencyTree.getSelectedAgency();
    //     portal_ZGBM.payProgress.show(selectAgency, true);
    // });
    $("#agencyTopProgressProjectPaymentIframe").prop("src", "http://192.168.10.11:8089/dss-web/ssoLogin/getEsen?resid=EBI$12$U9UURC0UPFMIDDI9JYDSQB8JXB91YTYM$1$T2RY9KDCYU2RUX7276XFXNUCCK2C6CYT.rpttpl&calcnow=true&showmenu=FALSE&showparams=false&tokenid=" + getTokenId());
    //$("#agencyTopProgressProjectPaymentIframe").prop("src", "http://www.nipic.com/index.html");

    // 单位支出进度
    portal_ZGBM.payProgress.show();
    ...
    window.onresize = function() {
        portal_ZGBM.topProgress.xsjdPosition();
    }

    // 右侧菜单树
    portal_ZGBM.rightAgencyTree.bf();
    portal_ZGBM.rightAgencyTree.show();
