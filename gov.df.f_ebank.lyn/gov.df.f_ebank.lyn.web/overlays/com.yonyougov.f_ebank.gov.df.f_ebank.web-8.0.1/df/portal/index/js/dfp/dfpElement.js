		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var o in _zhifu) {
					...
					html += '<tr onclick="window.parent.addTabToParent(&quot;' + menu_name + '&quot;,&quot;' + url + '&quot;);">';
					html += '<td>' + _onezhifu['name'] + '</td>' + 
							'<td>' + dfp.num2ThousandBreakNoDigit(_onezhifu['count']) + '</td>' + 
							'<td><span>' + dfp.num2ThousandBreak(money) + '</span></td>' +
							'</tr>';
				}
		...
		$("#" + id).find("div").each(function(j) {
			$(this).find("div").find("table").find("tbody").find("tr").each(function(i) {
				...
				if(i == 0)
					...
					var bgcolor = i % 2 == 0 ? "#f8f8f8" : "#FFFFFF"; // 奇偶行不同色
					$this.css("background-color", bgcolor).find("td").css("cursor", "pointer");
					$this.find("td:eq(1)").css({"color": "#0000FF", "font-weight": "bold"}); // 笔数
					$this.find("td:eq(2)").css({"color": "#F56A00", "font-weight": "bold"}); // 金额
					$this.find("td:eq(2)").find("span").css({"float": "right", "margin-right": "20px"});
					$this.on("mouseover", function() {
						...
						$this.find("td:eq(1)").css("color", "#0000FF");
