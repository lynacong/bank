...
var ptd_budget = {
	bf : function(){
		...
		$("#sure").on("click", function(){
			...
			// 是否政府采购
			var is_zfcg_chrid = $("#_portal_yusuan_table_gaoji_zfcg_select").val();
			...
			if(is_zfcg_chrid == '2') { // all
                condition += "";
			} else if(is_zfcg_chrid == '1') { // is zfcg
                condition += " and zfcgbs_code='1' ";
			} else if(is_zfcg_chrid == '0') {
...
//	    window.onmousewheel = document.onmousewheel = scrollFunc; 
		...
		$("#mainGeriContentDiv").bind("contextmenu", function(e){
			...
			if(browser == "IE"){
				...
				focus_row_canuse_money = ($(cur).find("td")[3].innerText).replace('\r\n', '');
		...
		for(var i=0; i<lengthOri; i++){
			for(var j=0; j<2; j++){
				var obj = {
					...
					"zcjd" : budgetTableDatadetailOri[i].zcjd
				};
	...
	tabledbClick : function(obj){
		...
		url += "&sum_id=" + focus_row_fromctrlid;
