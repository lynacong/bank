var fbUtil = fbUtil || {};
var fbUtil_util = fbUtil_util || {};
var fbUtil_re = fbUtil_re || {};
...
fbUtil = {
    ...
    commonData: function (options, type) {
        options = fbUtil_util.isNull(options) ? {} : options;
    ...
    pAjaxData: {
        ...
        caroleguid: function () {
            fbUtil_util.base64.encode(sessionStorage.select_role_guid == undefined ? "" : sessionStorage.select_role_guid);
        }
    },
    /**
     * 获取指定ls中指定字段值
    ...
    removeNumFromStr: function (str) {
        str = fbUtil_re.num.removeAll(str);
        str = fbUtil_re.trim(str);
        return str;
    },
    cookie: function () {
...
fbUtil_util = {
     ...
     */
    isGetValue: function () {
        var isfbUtilMenuLv3Ok = setInterval(function () {
            fbUtil_menu_lv3 = JSON.parse(sessionStorage.getItem("fbUtil_menu_lv3"));
            if (fbUtil_menu_lv3 != null && fbUtil_menu_lv3 != undefined) {
                clearTimeout(isfbUtilMenuLv3Ok);
            }
        }, 100);
        return JSON.parse(sessionStorage.getItem("fbUtil_menu_lv3"));
    },
    /**
     * 循环js对象，返回键值对对象
...
function fullUrlWithTokenid(url, param) {
    return fbUtil.completeUrlWithTokenid(url, param);
}
 ...
 * @params waitTime 等待次数，默认10次
 */
fbUtil.getMenuLv3 = function () {
    var fbUtil_menu_lv3 = '';
    while (true) {
        fbUtil_menu_lv3 = JSON.parse(sessionStorage.getItem('fbUtil_menu_lv3'));
        if (fbUtil_menu_lv3 != '')
            return fbUtil_menu_lv3;
    }
};

fbUtil.isArrayContain = function (arr, key) {
    for (var i in arr) {
 ...
 */
fbUtil.strSimilarityDegree = function (s1, s2) {
    // 分别获取url及全部参数对象
 ...
 * 获取url中指定参数
 */
fbUtil.getParamFromUrl = function (url, name) {
    if (url === '')
        return '';
 ...
 * @returns {string}
 */
fbUtil.fullUrl = function (u, t) {
	if(u.indexOf("?") == -1) {
		return u + "?" + "tokenid=" + fbUtil.tokenid();
	}
    return u + (u.indexOf("?") < u.length ? "&" : "?") + "tokenid=" + fbUtil.tokenid();
};
 ...
 */
fbUtil.strFormat2 = function (s, o) {
    if (arguments.length === 0) return '';
...
// ESEN 亿信单点登录
fbUtil.esen = {};
//fbUtil.esen.IP = "http://192.168.10.11:8089";
//fbUtil.esen.back = "/dss-web/ssoLogin/getEsen";
fbUtil.esen.IP = "http://192.168.10.11:9090";
fbUtil.esen.back = "/ESEN/showreport.do";
fbUtil.esen.params = "&calcnow=true&showmenu=FALSE&showparams=false";
fbUtil.esen.selfURL = {
    ...
    "YWCS_LEFT": "resid=EBI$12$U9UURC0UPFMIDDI9JYDSQB8JXB91YTYM$1$1NOAN8T9IBM6M35N5L3ZUIUCBSNMIR8W.rpttpl",
    ...
    "GKCX_TOP": "resid=EBI$12$U9UURC0UPFMIDDI9JYDSQB8JXB91YTYM$1$TINE8UOX3T7RMLLI9NSFR42X9KIY7SIM.rpttpl", // 国库查询
    ...
    "GKZFXD_DOWN_4": "resid=EBI$12$8UBKRKCQXLNN7NFUT0N9MATLM9UQUUTY$1$KAU1WYDZ8SD0U2IAWMDJUYKAEJALEJ65.rpttpl", // 支出预测
    "GKZFXD_DOWN_5": "" // 电子凭证
};
fbUtil.esen.url = function (key) {
    this.IP = fbUtil.ping.IP() || this.IP;
 ...
 * 多IP-Ping
 */
fbUtil.ping = function () {
    if (fbUtil.isNull(fbUtil.ping.IP())) {
        ...
        //     type: 'GET',
        //     data: {tokenid: fbUtil.tokenid()},
        //     dataType: 'json',
        //     async: false,
        //     success: function(data) {
        //         var urls = data.urls;
        //         if (fbUtil.isNull(urls) || urls.length == 0) {
        ...
        // 获取当前浏览器地址栏IP及端口
        sessionStorage.setItem('portal_ip_ping_0', fbUtil.urlIpPort());
        layer.close(pingLayer);
    }
};
fbUtil.ping.IP = function () {
 ...
 */
fbUtil.ping.addServerIP = function (url) {
    if (url.indexOf(fbUtil.esen.back) > -1)
        return fbUtil.ping.IP() + url;
    return url;
};
 ...
 * 注：不适用端口号后无内容的url
 */
fbUtil.urlIpPort = function () {
    var lc = window.document.location;
    var u = lc.href,
        p = lc.pathname;
    return u.substring(0, u.indexOf(p));
};

fbUtil.isNull = function (s) {
 ...
 * html字符匹配
 */
fbUtil.htmlCodeToSymbol = function (s) {
