...
var portalJZZF = {
    ...
    often: function () {
        ...
        dfpMenu.show("JZZF", "wen", null, "oftenUlWen");
        $("#oftenUlZhu").on("click", function () {
            window.parent.addTabToParent("集中支付查询", dfp.fullUrl(dfp.esen.url("JZZF_TOP")));
        });
                    ...
                    $(".agencyOperationBtn").each(function (i) {
                        if (urls[i] || urls[i] !== '') {
                            ...
                            $(this).on("mouseover", function () {
                                ...
                                $(this).css({"background": "#fff", "color": "#108ee9"});
                            });
                        } else {
...
$(function () {
    ...
    $("#fiscalSubImg").parent("span").on("click", function () {
        var fiscalParam = $("#fiscalInput").val();
        if (fiscalParam == "")
            layer.confirm('请输入要查询的内容', {
                btn: ['确定'] //按钮
                , title: '财政百度 提示'
                , shadeClose: true
                , cancel: function () {
