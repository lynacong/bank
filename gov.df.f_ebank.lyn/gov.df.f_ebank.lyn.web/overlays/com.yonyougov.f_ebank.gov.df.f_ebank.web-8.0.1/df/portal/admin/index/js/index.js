...
//	return "";
	...
	//return dfp_util.cookie.get("tokenid");
...
function addTabToParent(title, url){
	...
	if(url.indexOf("bigUlr_NewWin=1") > -1){
        window.open(url, 'maxwindow', "top=0,left=0,width=" +
            screen.availWidth +
            ",height=" +
            screen.availHeight +
            ",scrollbars=yes,toolbar=yes,location=no,directories=no,menubar=yes,resizable=yes,status=no");
		return;
	}
...
// 		}
		$.ajax({
			...
			success : function(data){
            	...
            	// 181006加入财务页面判定
				var dashboardUrlByRoleId = data.dashboardUrl;
				if (dashboardUrlByRoleId && dashboardUrlByRoleId.indexOf("/pf") > -1) {
					// 初始化财务登录
					var _tokenid = dfp_util.cookie.get("tokenid");
					$.ajax({
						url : "/pf/login/cloudLogin.do",
						type : 'GET',
						data: {username: Base64.decode($("#svUserId").val()), tokenid: _tokenid},
						async: false,
						success : function(data) {

							window.location.href = "/pf/portal/admin/index/index.html";
            	...
            	//20180930 厅长首页查询，做演示使用 把角色ID 写死。
            	if(data.publicParam.svRoleId == "32335"){
                  	 $("#topDanju").html('<span class="pay-detail-link">支付凭证查询</span>');
                  }
            	portal.menu.show();
	...
	for(var i=0; i<givenMenu.length; i++) {
		for(var j in dfp_menu_lv3) {
			...
			if(onemenu.code == givenMenu[i]){
				...
				url.push(fullUrlWithTokenid(onemenu.url)+'&menuid='+onemenu.guid+'&menuname='+dfp_re.space.removeAll(onemenu.name));
		...
		setInterval(function() {
			$.ajax({
				...
				success : function(data){
					var flag = data.flag;
					if(flag == 1){
						dfp_util.cookie.remove("tokenid", "/");
						// 云登录用户标记 login_from，云登录用户和本地登录用户退出跳转url有区别。local=本地登录，cloud=云登录。
						var login_from = sessionStorage.getItem("login_from"); 
						if (login_from == "cloud") {
							window.location.href = "/logout?redirect_uri=/";
						} else {
							window.location.href = "/";
						}
					}
