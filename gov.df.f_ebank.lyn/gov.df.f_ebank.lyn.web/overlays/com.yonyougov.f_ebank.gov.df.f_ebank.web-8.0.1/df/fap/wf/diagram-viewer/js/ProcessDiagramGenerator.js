...
var ProcessDiagramGenerator = {	
	...
	drawHighLights: function(processInstanceId,processDefinitionId) {
		...
		*/
		ActivitiRest.getHighLights(processInstanceId,processDefinitionId, this._drawHighLights);
	},
	...
	drawDiagram: function(processDefinitionId,processInstanceId) {
		...
		//if (processDefinitionId.indexOf(":") < 0) {_20170630
	...
	showFlowInfo: function(flow){/*  20170704
		...
		var TPL_FLOW_INFO = '<div>{flow}</div>' 
				+ '<div><b>起点</b>: {sourceActivityId}</div>'
				+ '<div><b>终点</b>: {destinationActivityId}</div>'
				+ '<div><b>是否默认</b>: {isDefault}</div>'
				+ '<div><b>是否有条件</b>: {isConditional}</div>'
				+ '<div><b>是否高亮</b>: {isHighLighted}</div>';
		var tpl = Lang.sub(TPL_FLOW_INFO, values);
		//console.log("info: ", tpl);
		diagramInfo.html(tpl);
	*/},
	
	showActivityInfo: function(activity){/*  20170704
		var diagramInfo = $("#" + this.options.diagramInfoId);
		if (!diagramInfo) return;
		...
		var TPL_ACTIVITY_INFO = '' 
				+ '<div><b>活动ID</b>: {activityId}</div>'
				+ '<div><b>活动名称</b>: {name}</div>'
				+ '<div><b>类型</b>: {type}</div>';
		var TPL_CALLACTIVITY_INFO = ''
				+ '<div><b>折叠</b>: {collapsed}</div>'
				+ '<div><b>流程定义ID</b>: {processDefinitonKey}</div>';
	...
	*/
		...
		//{
			var taskAssignment=activity.getProperty("assignment"+tasknum);
			var taskcreateTime=activity.getProperty("createTime"+tasknum);
			...
			{
				showMessage=showMessage+ '<div><div>操作人:' +taskAssignment;
				height=height+14;
			}
			if(taskcreateTime!=null)
			{
				var operatorType=activity.getProperty("operatorType");
				if(operatorType=="done"){
					showMessage=showMessage+ '<div><div>操作时间: '+taskcreateTime;
					height=height+14;
				}else if(operatorType=="current"){
					showMessage=showMessage+ '<div><div>创建时间: '+taskcreateTime;
					height=height+14;
				}
		...
		var TPL_ACTIVITY_INFO = '' 
				//+ '<div id="showActMessage" style="padding:10px;position: absolute; top:'+maxY+'px; left:'+left+'px; border:1px solid; width: '+width+'px; height: '+height+'px;background-color: #FFFFFF"><font size="1px" color="#333"> <div>活动ID: {activityId}</div>'
				+ '<div id="showActMessage" style="padding:10px;position: absolute; top:'+maxY+'px; left:'+left+'px; border:1px solid; width: '+width+'px; height: '+height+'px;background-color: #FFFFFF"><font size="1px" color="#333">'
				+ '<div>活动名称: {name}</div>'
		...
		var template = TPL_ACTIVITY_INFO+showMessage;
		if (activity.getProperty("type") == "callActivity") {
			values.collapsed = activity.getProperty("collapsed");
			if(values.collapsed)
				values.collapsed="是";
