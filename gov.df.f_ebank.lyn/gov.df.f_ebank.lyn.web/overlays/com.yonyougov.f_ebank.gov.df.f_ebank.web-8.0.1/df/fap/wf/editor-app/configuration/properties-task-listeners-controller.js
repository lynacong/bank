...
var KisBpmTaskListenersPopupCtrl = [ '$scope', '$q', '$translate','$timeout', function($scope, $q, $translate,$timeout) {
    ...
    if ($scope.property.value !== undefined && $scope.property.value !== null
        ...
        for (var i = 0; i < $scope.taskListeners.length; i++)
        {
            ...
            switch($scope.taskListeners[i].event){
	    		case "create":
	    			$scope.taskListeners[i].event_displayname = ORYX.I18N.Listeners.Create;
	    			break;
	    		case "assignment":
	    			$scope.taskListeners[i].event_displayname = ORYX.I18N.Listeners.Assignment;
	    			break;
	    		case "complete":
	    			$scope.taskListeners[i].event_displayname = ORYX.I18N.Listeners.Complete;
	    			break;
	    		case "delete":
	    			$scope.taskListeners[i].event_displayname = ORYX.I18N.Listeners.Delete;
	    			break;
	    		default:
	    			break;
            }
    ...
    var fieldPromise = $translate('PROPERTY.EXECUTIONLISTENERS.FIELDS.STRINGVALUE');
    
    $q.all([eventPromise, implementationPromise, namePromise, fieldPromise]).then(function(results) { 
        $scope.labels.eventLabel = results[0];
        $scope.labels.implementationLabel = results[1];
        $scope.labels.nameLabel = results[2];
        $scope.labels.fieldLabel = results[3];
        ...
        $scope.gridOptions = {
            ...
            columnDefs: [
            	{ field: 'event', displayName: $scope.labels.eventLabel, visible:false},
            	{ field: 'event_displayname', displayName: $scope.labels.eventLabel },
        ...
        $scope.gridFieldOptions = {
            ...
            columnDefs: [{ field: 'name', displayName: $scope.labels.nameLabel },
                { field: 'implementation', displayName: $scope.labels.fieldLabel}]
        };
    });
    
    $scope.eventChanged = function(){
    	switch($scope.selectedListeners[0].event){
    		case "create":
    			$scope.selectedListeners[0].event_displayname = ORYX.I18N.Listeners.Create;
    			break;
    		case "assignment":
    			$scope.selectedListeners[0].event_displayname = ORYX.I18N.Listeners.Assignment;
    			break;
    		case "complete":
    			$scope.selectedListeners[0].event_displayname = ORYX.I18N.Listeners.Complete;
    			break;
    		case "delete":
    			$scope.selectedListeners[0].event_displayname = ORYX.I18N.Listeners.Delete;
    			break;
    		default:
    			break;
    	}
    ...
    $scope.listenerDetailsChanged = function(type) {
        if (type=="class"&&$scope.selectedListeners[0].className !== '')
        {
            $scope.selectedListeners[0].implementation = $scope.selectedListeners[0].className;
        }
        else if (type=="expression"&&$scope.selectedListeners[0].expression !== '')
        {
            $scope.selectedListeners[0].implementation = $scope.selectedListeners[0].expression;
        }
        else if (type=="delegateexpression"&&$scope.selectedListeners[0].delegateExpression !== '')
        {
    ...
    $scope.addNewListener = function() {
        $scope.taskListeners.push({ event : 'create',
        	event_displayname : ORYX.I18N.Listeners.Create,
            implementation : '',
            className : '',
            expression: '',
            delegateExpression: '',
            clazz:'class'});
        $timeout(function(){
                    $scope.gridOptions.selectItem($scope.taskListeners.length - 1, true);   
                }, 100);
    };
    ...
    $scope.addNewField = function() {
        if ($scope.selectedListeners.length > 0)
        {
            ...
            $scope.selectedListeners[0].fields.push({ name : 'fieldName',
                ...
                string: ''});
            $timeout(function(){
                    $scope.gridFieldOptions.selectItem($scope.selectedListeners[0].fields.length - 1, true);   
                }, 100);
        }
    };
