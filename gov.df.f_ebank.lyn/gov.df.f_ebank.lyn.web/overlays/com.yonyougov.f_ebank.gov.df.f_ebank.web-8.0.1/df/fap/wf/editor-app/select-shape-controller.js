...
var KisBpmShapeSelectionCtrl = [ '$rootScope', '$scope', '$timeout', '$translate', function($rootScope, $scope, $timeout, $translate) {
    ...
    for (var i = 0; i < $scope.morphShapes.length; i++)
    {
    	...
    	{
    		var flag=0;
    		for(var j=0;j<$scope.availableMorphShapes.size();j++)
    		{
    			if($scope.availableMorphShapes[j].id==$scope.morphShapes[i].id)
    				flag=1;
        ...
        if ($scope.selectedMorphShapes.length > 0) 
        {
        	var MorphTo = ORYX.Core.Command.extend({
    			...
    			execute: function(){
    				...
    				if(shape.parent._stencil._jsonStencil.id.indexOf("EventSubProcess")==-1
    						&&stencil._jsonStencil.id.indexOf("StartErrorEvent")!=-1) 
    					return;
    				...
    				//add by zhuyjh 切换类型后，属性值清空 ,只保留parent属性，定位元素在子流程中的位置
    				var parent;
    				for(var i=0;i<serialized.size();i++)
    					{
    						var temp=serialized.pop();
    						if(temp.name=="parent")
    							{
    							parent=temp;
    							}
    		...
    		if(command.shape.properties["oryx-forcompensation"]==true)
			{
