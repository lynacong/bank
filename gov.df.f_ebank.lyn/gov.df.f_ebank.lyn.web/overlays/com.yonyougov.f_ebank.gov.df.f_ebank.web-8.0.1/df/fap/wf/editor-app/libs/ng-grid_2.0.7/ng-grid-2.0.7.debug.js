...
var ngSearchProvider = function ($scope, grid, $filter) {
    ...
    $scope.$watch('filterText', function(a){
        if (!self.extFilter) {
        	...
        	$scope.pagingOptions.currentPage = 1;
        	$scope.filterOptions.filterText = a;
...
var ngSelectionProvider = function (grid, $scope, $parse) {
    ...
    self.getSelection = function (entity) {
        ...
        if (grid.config.primaryKey) {
            ...
            angular.forEach(self.selectedItems, function (c, i) {
    ...
    self.setSelection = function (rowItem, isSelected) {
        if(grid.config.enableRowSelection){
            if (!isSelected) {
            	var index = -1;
            	if (grid.config.primaryKey) {
		            var val = self.pKeyParser(rowItem.entity);
		            angular.forEach(self.selectedItems, function (c, i) {
		                if (val === self.pKeyParser(c)) {
		                    index = i;
		                }
		        	...
		        	index = self.selectedItems.indexOf(rowItem.entity);
		        }
                if (index !== -1) {
                    self.selectedItems.splice(index, 1);
                }
...
angular.module("ngGrid").run(["$templateCache", function($templateCache) {
  ...
  $templateCache.put("cellTemplate.html",
    "<div class=\"ngCellText\" ng-class=\"col.colIndex()\"><span ng-cell-text title=\"{{COL_FIELD CUSTOM_FILTERS}}\">{{COL_FIELD CUSTOM_FILTERS}}</span></div>"
  );
  ...
  $templateCache.put("footerTemplate.html",
    ...
    "            <span class=\"ngLabel\" style=\"display:inline-block\">{{i18n.ngTotalItemsLabel}}{{maxRows()}}</span><span ng-show=\"false\" class=\"ngLabel\">({{i18n.ngShowingItemsLabel}}{{totalFilteredItemsLength()}})</span>" +
    "        </div>" +
    ...
    "            <input class=\"ngPagerCurrent\" min=\"1\" max=\"{{maxPages()}}\" type=\"text\" style=\"width:50px; height: 24px; margin-top: 1px; padding: 0 4px;\" value=\"{{pagingOptions.currentPage}}\" ng-model=\"pagingOptions.currentPage\"/>" +
