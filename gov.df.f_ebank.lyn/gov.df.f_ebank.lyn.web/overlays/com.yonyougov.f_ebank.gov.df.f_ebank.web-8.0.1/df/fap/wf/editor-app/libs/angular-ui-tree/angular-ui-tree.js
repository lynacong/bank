  ...
  angular.module('ui.tree')
    .controller('TreeController', ['$scope', '$element', '$window', '$attrs', 'treeConfig', 'keys',
      function ($scope, $element, $window, $attrs, treeConfig, keys) {
        ...
        //modify by chouhl
        if(typeof($scope.$parent.property.treeMultiSelect) == "boolean"){
        	$scope.multiSelect = $scope.$parent.property.treeMultiSelect;
        }else if(typeof($scope.$parent.property.multiSelect) == "boolean"){
        	$scope.multiSelect = $scope.$parent.property.multiSelect;
        }else{
        ...
        $scope.select = function() {
          if (!$scope.selected && $scope.$treeScope.$callbacks.select($scope)) {
            ...
            if(!$scope.$treeScope.multiSelect){
            	var element = $scope.$treeScope.$selecteds.pop();
            	if(element){
            		element.scope().unselect();
            	}
            }
            //
        ...
        return {
          ...
          link: function(scope, element, attrs, controllersArr) {
            ...
            var toggleSelect = function(e) {
              ...
              //modify by chouhl if (!checkParentSelect(scope)) {
              scope.$apply(scope.toggleSelected);
              if(scope.selected){
              	setChildSelect(scope);
              }
              //}
            };
            
            //add by chouhl
            var toggleSingleSelect = function(e) {
              e.preventDefault();
              e.stopPropagation();
            ...
            //add by chouhl
            var setChildSelect = function(elementScope) {
              elementScope = (angular.isUndefined(elementScope)) ? scope : elementScope;

              if (elementScope.hasChild()) {
                angular.forEach(elementScope.childNodes(), function(childNodeScope) {
                  childNodeScope.$apply(childNodeScope.select);
                  setChildSelect(childNodeScope);
                });
              }
            };
            //
            ...
            var bindDrag = function() {
              element.bind('touchstart mousedown', function (e) {
              	if(e.target.tagName.toLowerCase() == 'div'){
	                ...
	                //add by chouhl
              		if("treegrid" == scope.$treeScope.$parent.property.referenceType){
              			var selecteds = scope.$treeScope.$selecteds;
    			    	var len = selecteds.length; 
    					if (len > 0) {
    						scope.$treeScope.$parent.params = {};
    						scope.$treeScope.$parent.params.treeSelecteds = [];
    						for(var i = 0; i < len; i++){
    							scope.$treeScope.$parent.params.treeSelecteds.push(selecteds[i].scope().$modelValue);
    						}
    						scope.$treeScope.$parent.getPagedDataAsync(scope.$treeScope.$parent.pagingOptions.pageSize, scope.$treeScope.$parent.pagingOptions.currentPage);
    			        }else{
    			        	scope.$treeScope.$parent.clearData();
    			        }
              		}
              	}
              });
