    ...
    .controller('ToolbarController', ['$scope', '$http', '$modal', '$q', '$rootScope', '$translate', '$location', function ($scope, $http, $modal, $q, $rootScope, $translate, $location) {
        ...
        $scope.editorFactory.promise.then(function() {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_EXECUTE_COMMANDS, function( evt ){
                ...
                var oryx_name =evt.commands[0] && evt.commands[0].key && evt.commands[0].key==="oryx-name";
                for(var i = 0; i < $scope.items.length; i++) 
        		{
                    var item = $scope.items[i];
                    if (item.action === 'KISBPM.TOOLBAR.ACTIONS.undo' && !oryx_name)
                    {
                    	item.enabled = true;
                    }
                ...
                var toolbarBtnUndo = document.getElementById("toolbar-btn-undo");
       		 	if ($scope.undoStack.length > 0 && toolbarBtnUndo.disabled && !oryx_name){
       		 		toolbarBtnUndo.disabled=false;
                }
