  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                //begin_修改加载设计器时，加载的数据,第一次不保存_2017_04_19
		         
                   //var newModelData = KISBPM.URL.getModel(newModelData);
                   var newModelData = EDITOR.UTIL.getUrlParameter('newModelData');
	                   if( newModelData !=null){
	                	   var newJson=JSON.parse(newModelData);
	                	    //begin_将mainTable放在root中
	                	     var mainTableName=(newJson.model.properties.multiinstance_maintablename).split(" ")[0];
	                	     $rootScope.mainTableName_condition=mainTableName;
	                	   //end
	                	   
	                	   var tmp=newJson.model.stencilset;
	                	   var namespace=tmp.namespace;
	                	   namespace=namespace.substring(0,namespace.length-1);
	                	   tmp.namespace=namespace;
	                	   newJson.model.stencilset=tmp;
	                	   $rootScope.editor = new ORYX.Editor(newJson);
                           $rootScope.modelData = angular.fromJson(newJson);
                           $rootScope.editorFactory.resolve();
	                   }else{
	                	   $http({method: 'GET', url: modelUrl}).
	                       success(function (data, status, headers, config) {
	                    	 //begin_将mainTable放在root中
		                	     var mainTableName=(data.model.properties.multiinstance_maintablename).split(" ")[0];
		                	     $rootScope.mainTableName_condition=mainTableName;
		                	   //end
  		      ...
  		      //end_修改加载设计器时，加载的数据,第一次不保存_2017_04_19
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    jQuery('#paletteHelpWrapper').height(totalAvailable-20);
	                    jQuery('#paletteSection').height(totalAvailable-15);
	                    jQuery('#canvasHelpWrapper').height(jQuery(window).height()-36);
	                    jQuery('#propertiesBody').height(jQuery(window).height()-70);
	                    
	                    
	                    //canvas.width("1200");
	                    canvas.height(jQuery(window).height()-36);
	                    //jQuery('#canvasSection').height(jQuery(window).height()+300);
	                    //jQuery('#paletteSection').height(totalAvailable+370);
	                   /* jQuery('#paletteSection').height(totalAvailable+370);
	                    jQuery('#propertiesInnerBody').height(totalAvailable);
	                    jQuery('#canvasSection').height(jQuery(window).height()+100);*/
	                   ...
	                   /* if(actualWidth < canvas[0].clientWidth) {
	                    ...
	                    var north = jQuery('#canvas-shrink-N');
	                    north.css('top', canvasTop + iconOffset + 'px');
	                    north.css('left', canvasLeft + 10 + (canvasWidth - iconWidth) / 2 + 'px');

	                    var south = jQuery('#canvas-shrink-S');
	                    south.css('top', (canvasTop + canvasHeight - iconOffset - iconCenterOffset) +  'px');
	                    south.css('left', canvasLeft +10 + (canvasWidth - iconWidth) / 2 + 'px');

	                    var east = jQuery('#canvas-shrink-E');
	                    east.css('top', canvasTop + 10 + (canvasHeight - iconWidth) / 2 +  'px');
	                    east.css('left', (canvasLeft + canvasWidth - iconOffset - iconCenterOffset) + 'px');

	                    var west = jQuery('#canvas-shrink-W');
	                    west.css('top', canvasTop + 10 + (canvasHeight - iconWidth) / 2 + 'px');
	                    west.css('left', canvasLeft + iconOffset + 'px');
	                });
