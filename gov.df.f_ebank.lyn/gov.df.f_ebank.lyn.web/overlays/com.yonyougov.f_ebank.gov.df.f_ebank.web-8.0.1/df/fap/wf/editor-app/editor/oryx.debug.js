...
ORYX.CONFIG.TYPE_KISBPM_MULTIINSTANCE_MODEL =	"kisbpm-multiinstancemodel";
				...
				if(tspan.textContent.trim() === "") {
					...
					switch (this._verticalAlign) {
						case 'bottom':
							...
							break;
						case 'middle':
							dy = -(length / 2.0 - index - 1) * (fontSize+5);
...
ORYX.Core.StencilSet.Rules = {
	...
	canConnect: function(args) {	
		...
		if(args.edgeStencil!=null&&args.edgeStencil._jsonStencil.id!=null&&args.edgeStencil._jsonStencil.id.indexOf("Association")!=-1&&args.targetShape!=null&&!(args.targetShape.properties["oryx-forcompensation"]==true))
			return false;
		// init arguments
		if(args.sourceShape) {
			args.sourceStencil = args.sourceShape.getStencil();
...
ORYX.Core.StencilSet.StencilSet = Clazz.extend({
    ...
    construct: function(source, modelMetaData, editorId){
		...
		if(ORYX.CONFIG.BACKEND_SWITCH) {
			...
			new Ajax.Request(window.location.pathname.substring(0,window.location.pathname.indexOf("/",1))+'/service'+ '/editor/stencilset?version=' + Date.now(), {
	            asynchronous: false,
	            method: 'get',
...
ORYX.Editor = {
	...
	*/
	...
	deleteShape: function(shape) {
		...
		for(var i=0;i<shape.children.size();i++)
		{
			if(shape.children[i] instanceof ORYX.Core.Shape)
				for(var j=0;j<shape.children[i].incoming.size();j++)
					shape.parent.remove(shape.children[i].incoming[j]);
		}
	...
	*/
	...
	handleEvents: function(event, uiObj) {
		...
		if(this.dragEnable!="mousedown"||(this.dragEnable!=null&&this.dragEnable!=undefined&&this.dragEnable&&this.dragEnable=="mousedown"))
		{
					...
					if (property != undefined) {
						...
						if(property.type() == ORYX.CONFIG.TYPE_CHOICE) {
							...
							property.refToView().each((function(ref) {
								...
								if (property.type() === ORYX.CONFIG.TYPE_KISBPM_MULTIINSTANCE||property.type() === ORYX.CONFIG.TYPE_KISBPM_MULTIINSTANCE_MODEL)
								{
									if (ref === "multiinstance"||ref==="multiinstancemodel") {
										
										var svgElemParallel = this.node.ownerDocument.getElementById(this.id + 'parallel');
										if(svgElemParallel) 
										{
											if (prop === 'Parallel'||prop === 'Sign'||prop === 'Grab')
											{
...
ORYX.Core.Node = {
    ...
    _init: function(svgDocument){
        ...
        if (svgNode.attributes.getNamedItem("title").nodeValue.indexOf("边界")!=-1) {
...
ORYX.Core.Edge = {
    ...
    refresh: function(){
        ...
        if(this.getSource() && this.getSource().parent && this.getSource().parent._offset){
        	_offset = this.getSource().parent._offset;
        	_time = this.getSource().parent._time;
        }else if(this.getTarget() && this.getTarget().parent && this.getTarget().parent._offset){
        	_offset = this.getTarget().parent._offset;
        	_time = this.getTarget().parent._time;
        }else if(this.incoming && this.incoming.length > 0 && this.incoming[0].parent && this.incoming[0].parent._offset){
        	_offset = this.incoming[0].parent._offset;
        	_time = this.incoming[0].parent._time;
        }else if(this.outgoing && this.outgoing.length > 0 && this.outgoing[0].parent && this.outgoing[0].parent._offset){
        	_offset = this.outgoing[0].parent._offset;
        	_time = this.outgoing[0].parent._time;
        }
        ...
        this._paths.each((function(path, index){
            ...
            for (var i = 1; i < dockers.length; i++) {
            	/*add by chouhl*/
            	if(_offset && i < dockers.length - 1){
            		if(!dockers[i]._time || _time > dockers[i]._time){
            			dockers[i].bounds.moveBy(_offset);
            			dockers[i]._time = _time;
            		}
            	}
            	/**/
...
ORYX.Plugins.RenameShapes = Clazz.extend({
	...
	actOnDBLClick: function(evt, shape){
		...
		props = props.findAll(function(item){ return !item.readonly() &&  (item.type() == ORYX.CONFIG.TYPE_STRING || item.type() == ORYX.CONFIG.TYPE_EXPRESSION || item.type() == ORYX.CONFIG.TYPE_DATASOURCE||item.type() == ORYX.CONFIG.TYPE_TEXT ); });
		...
		var width		= Math.min(Math.max(90, shape.bounds.width()), 200);
		...
		//add by chouhl
		if(jQuery("#shapeTextInput").val()){
			var len = jQuery("#shapeTextInput").val().length;
			if(len && len > 0 && jQuery("#shapeTextInput")[0].setSelectionRange){
				jQuery("#shapeTextInput")[0].setSelectionRange(len, len);
			}
		}
		//
...
ORYX.Plugins.Arrangement = ORYX.Plugins.AbstractPlugin.extend({
	...
	alignShapes: function(way) {
		...
		var removeDockerCommandClass = ORYX.Core.Command.extend({
            construct: function(edge, docker, facade){
                this.edge = edge;
                this.docker = docker;
                this.pos;
                this.facade = facade;
        ...
        var isCommandNotExist = function(dockerId){
        	var temp = true;
			var len = commands.length;
			for(var i = 0; i < len; i++){
				if(commands[i] && commands[i].docker && commands[i].docker.id && commands[i].docker.id == dockerId){
					temp = false;
					break;
				}
			}
		...
		if(way==48)
		{
			var flagSubProcess=0;
			var flagDif=0;
			for(var i=0;i<elements.length;i++)
			{
				if(elements[i]._stencil._jsonStencil.id.indexOf("SubProcess")!=-1)
					flagSubProcess=1;
				else
					flagDif=1;
			}
			if(flagSubProcess==1&&flagDif==1) { return; }
		}
		else if(ORYX.CONFIG.EDITOR_ALIGN_CENTER == way){
			var isContinue = true;
			var bounds = new Array();
			jump:for(var i = 0; i < elements.length; i++){
				var bound = elements[i].bounds;
				for(var j = 0; j < bounds.length; j++){
					if((bound.a.y >= bounds[j].a.y && bound.a.y <= bounds[j].b.y) || (bound.b.y >= bounds[j].a.y && bound.b.y <= bounds[j].b.y)){
						isContinue = false;
						break jump;
					}
				}
			...
			var minx = elements[0].bounds.a.x;
			var miny = elements[0].bounds.a.y;
			var maxx = elements[0].bounds.b.x;
			var maxy = elements[0].bounds.b.y;
			for(var i = 1; i < elements.length; i++){
				var bound = elements[i].bounds;
				minx = bound.a.x < minx ? bound.a.x : minx;
				miny = bound.a.y < miny ? bound.a.y : miny;
				
				maxx = bound.b.x > maxx ? bound.b.x : maxx;
				maxy = bound.b.y > maxy ? bound.b.y : maxy;
			}
			
			for(var i = 0; i < elements.length; i++){
				var incoming = elements[i].incoming;
				for(var j = 0; j < incoming.length; j++){
					if(!incoming[j].isDocked()){
						continue;
					}
					
					var dockers = incoming[j].dockers;
					var dockers_len = dockers.length;
					
					var isInside = false;
					
					var bound = dockers[0].bounds;
					if((bound.a.x >= minx && bound.a.y >= miny) && (bound.b.x <= maxx && bound.b.y <= maxy)){
						isInside = true;
					}else{
						isInside = false;
					}
					
					bound = dockers[dockers_len - 1].bounds;
					if((bound.a.x >= minx && bound.a.y >= miny) && (bound.b.x <= maxx && bound.b.y <= maxy)){
						isInside = true;
					}else{
						isInside = false;
					...
					if(isInside){
						for(var k = dockers_len - 2; k > 0; k--){
							if(isCommandNotExist(dockers[k].id)){
								var removeCommand = new removeDockerCommandClass(incoming[j], dockers[k], this.facade);
				...
				var outgoing = elements[i].outgoing;
				for(var j = 0; j < outgoing.length; j++){
					if(!outgoing[j].isDocked()){
						continue;
					}
					
					var dockers = outgoing[j].dockers;
					var dockers_len = dockers.length;
					...
					if(isInside){
						for(var k = dockers_len - 2; k > 0; k--){
							if(isCommandNotExist(dockers[k].id)){
								var removeCommand = new removeDockerCommandClass(outgoing[j], dockers[k], this.facade);
								commands.push(removeCommand);
							}
						}
					}
		...
		}else if(ORYX.CONFIG.EDITOR_ALIGN_MIDDLE == way){
			var isContinue = true;
			var bounds = new Array();
			jump:for(var i = 0; i < elements.length; i++){
				var bound = elements[i].bounds;
				for(var j = 0; j < bounds.length; j++){
					if((bound.a.x >= bounds[j].a.x && bound.a.x <= bounds[j].b.x) || (bound.b.x >= bounds[j].a.x && bound.b.x <= bounds[j].b.x)){
						isContinue = false;
						break jump;
					}
				}
...
ORYX.Plugins.DragDropResize = ORYX.Plugins.AbstractPlugin.extend({
	...
	*/
	handleMouseMove: function(event) {
		...
		/*if(Math.abs(event.mozMovementX)+Math.abs(event.mozMovementX)<5)
			return;*/
		...
		if( !this.isAttachingAllowed){
			if( this.isAddingAllowed &&!(this.containmentParentNode._stencil._jsonStencil.id.indexOf("#SubProcess")!=-1&&this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartErrorEvent")!=-1)
					&&!(this.containmentParentNode._stencil._jsonStencil.id.indexOf("#EventSubProcess")!=-1&&(this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartTimerEvent")!=-1||this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartNoneEvent")!=-1))) {
				
				if(this.containmentParentNode.parent==undefined&&this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartErrorEvent")!=-1)
				{
					document.getElementById("canvasHelpWrapper").style.cursor="no-drop";
				}
				else if(this.containmentParentNode._stencil._jsonStencil.id.indexOf("SubProcess")!=-1&&this.currentShapes[0]._stencil._jsonStencil.id.indexOf("Boundary")!=-1)
				{
					document.getElementById("canvasHelpWrapper").style.cursor="no-drop";
				}
				else
...
//	},
	...
	checkRules : function(options) {
		...
		if( checkIfAttachable ){
			...
			if(this.containmentParentNode._stencil._jsonStencil.id.indexOf("SubProcess")!=-1
					&&this.toMoveShapes[0]._stencil._jsonStencil.id.indexOf("Boundary")!=-1) 
	...
	refreshSelectedShapes: function() {
		...
		if(this.containmentParentNode!=null&&!((this.containmentParentNode.parent==undefined||this.containmentParentNode._stencil._jsonStencil.id.indexOf("#SubProcess")!=-1)&&this.toMoveShapes.size()==1&&
				this.toMoveShapes[0]._stencil._jsonStencil.id.indexOf("StartErrorEvent")!=-1))
			{
				if(!(this.containmentParentNode._stencil._jsonStencil.id.indexOf("#EventSubProcess")!=-1&&(this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartTimerEvent")!=-1||this.currentShapes[0]._stencil._jsonStencil.id.indexOf("#StartNoneEvent")!=-1)))
				{	if(!(this.containmentParentNode._stencil._jsonStencil.id.indexOf("SubProcess")!=-1&&this.currentShapes[0]._stencil._jsonStencil.id.indexOf("Boundary")!=-1))
					{
						if(!(this.containmentParentNode.parent==undefined&&this.currentShapes[0]._stencil._jsonStencil.id.indexOf("Boundary")!=-1))
							{
		...
		if(this.isResizing) {
			...
			var commandClass = ORYX.Core.Command.extend({
				...
				update:function(offset){
					...
					//add by zhuyjh  subprocess 拖拽左上角
					if(allEdges.size()==0){
						for(var i=0;i<this.shape.children.size();i++){
							if(this.shape.children[i] instanceof ORYX.Core.Node){
								allEdges = allEdges.concat(this.shape.children[i].getIncomingShapes())
								// Remove all edges which are included in the selection from the list
								.findAll(function(r){ return r instanceof ORYX.Core.Edge }.bind(this))
							}
						}
						for (var j = 0; j < allEdges.length; j++) {
							for (var k = 1; k < allEdges[j].dockers.length-1; k++) {
								var docker = allEdges[j].dockers[k];
								if (!docker.getDockedShape()) {
...
ORYX.Plugins.Resizer = Clazz.extend({
	...
	handleMouseMove: function(event) {
		if(Math.abs(event.mozMovementX)>2||Math.abs(event.mozMovementY)>2
				||Math.abs(event.webkitMovementX)>2||Math.abs(event.webkitMovementY)>2)
			document.getElementById("canvasHelpWrapper").style.cursor="default";
		if(!this.dragEnable) { return }
...
ORYX.Core.Command.Move = ORYX.Core.Command.extend({
	...
	move:function(offset, doLayout){
		...
		for(var i=0; i<this.moveShapes.length ;i++){
			...
			/*add by chouhl*/
			/*if(value._stencil && (value._stencil.idWithoutNs() == "SubProcess" || value._stencil.idWithoutNs() == "EventSubProcess")){
				value._offset = offset;
				value._time = (new Date()).getTime();
			}*/
			/**/
        ...
        var commandClass = ORYX.Core.Command.extend({
            ...
            execute: function(){
                if (this.addEnabled) {
					...
					/*add by chouhl*/
					this.docker._time = (new Date()).getTime();
					/**/
                }
