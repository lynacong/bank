define(
		[ 'jquery', 'knockout', '/df/fap/system/config/ncrd.js',
				'text!fap/system/config/billno/editAdd._html',
				...
				if(viewModel.editDataTable.getSimpleData()){
					...
					if(row){
						...
						if (index > 0) {// 判断当前行是否为第一行
							var arr = [], arr_before = [];
							var focus_row = viewModel.editDataTable.getFocusRow(index);// 取出当前焦点行
			...
			viewModel.editDown = function() {
				if(viewModel.editDataTable.getSimpleData()){
					...
					if(row){
						...
						// var rowNum =viewModel.editDataTable.totalRow();//获取表格的行数
...
//					alert("当前表格没有可删除的数据！");
			...
			viewModel.add_ruleItem = function() {
				// viewModel.ruleItem_dataTable.addRow();// 增加一行
				viewModel.ruleItem_dataTable.createEmptyRow();
				setLineno('eles');//设置序号
//				getElesData();			
			};
			//  增加/修改单号规则 保存数据到edit页面
			viewModel.save_ruleItem = function() {
				...
				if (checkRule(line_type, line_format, ele_code, level_num,init_value) && checkEleNull() && checkEleOnly()) {// 校验
					if (title == 'addRule') {// 增加单号规则项
			...
			function show(billId, billnoRuleData, callback) {
				...
				if (billId == null) {// 没有选中树节点
					...
					for ( var i = 0; i < rows.length; i++) {// 下拉列表名称和value值转换
						...
						/*
						 * var eles=rows[i].getValue('eles'); console.log(eles);
						 * viewModel.ruleItem_dataTable.setSimpleData(eles,{unSelect:true});
						 */
					}
			...
			var getTypeNameByVal= function (type) {
				...
				/*
				 * switch(line_type){ case 0:
				 * rows[i].setValue('line_type_name','常量'); break; case 1:
				 * rows[i].setValue('line_type_name','日期'); break; case 2:
				 * rows[i].setValue('line_type_name','要素'); break; case 3:
				 * rows[i].setValue('line_type_name','要素自增序列'); break;
				 * default:break; }
				 */
			}
			...
			// 校验编码的唯一性
			var checkBillCode= function (billCode, arr) {
				var flag = true;
...
//					alert("单号规则编码不规范，请按照三位一节输入单号规则编码！");
			...
			viewModel.btnSaveClick = function() {
				if (onCloseCallback.save) {
					...
					var billnorulelines = viewModel.editDataTable
							.getSimpleData({
								'fields' : [ 'line_no', 'line_type',
										'line_format', 'ele_code', 'level_num',
										'init_value', 'eles' ]
							});					
...
//					console.log(JSON.stringify(data));
					if (checkBill1(billnorule_code, billnorule_name)
							...
							&& checkBillCode(billnorule_code, treeData)
							&& checkBill2()) {// 表单校验
						$.ajax({
							type : 'POST',
				...
				$.ajax({
					...
					success : function(result) {
						...
						if (data != null) {
							...
							viewModel.comElesArr.splice(0,viewModel.comElesArr.length);
							for ( var i = 0; i < data.length; i++) {
