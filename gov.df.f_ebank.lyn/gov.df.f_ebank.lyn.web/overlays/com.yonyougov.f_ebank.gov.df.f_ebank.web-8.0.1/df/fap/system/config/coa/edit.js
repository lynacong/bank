 ...
 */
define(['jquery', 'knockout', 'text!fap/system/config/coa/edit._html', 'grid', 'bootstrap', 'uui', 'tree', 'ip'],
...
//                        showTree(node);
        ...
        function updateView_1(element) {
            ...
            var
                ele,
                tree,
                emptyTree = [{codename: '', chr_id: ''}],
                emptyEle = null,
                max_level,
                defaultValueName = '',
                isExist = s.coaDetailList.some(function (coa) {
                    if (coa.eleCode === element.ele_code) {
                        ele = coa
                        return true
                    }
                });
            tree = window.els.filter(function (el) {
                return el.ele_code === element.ele_code
            })[0].tree
            tree = tree || []
            ...
            if (isExist) { // 元素已经存在
                ...
                viewModel.eleObj.defaultValue(ele.defaultValue)
                tree.some(function (item) {
                    if (item.chr_id === ele.defaultValue) {
                        defaultValueName = item.codename
                        return true
                    }
                })
            } else { // 元素不存在
                ...
                viewModel.eleObj.defaultValue(emptyEle.defaultValue)
            }
            viewModel.eleObj.defaultValueName(defaultValueName)
            s.inChangeEle = false
        }
        ...
        var getEleTree = function (code) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.errorCode == "0") {
                        result.data[0].row.map(function (e) {
                            e["codename"] = e.chr_code + ' ' + e.chr_name;
                            return e;
                        });
                        ...
                        viewModel.eleObj.defaultValueName('')
                        ip.ipInfoJump("错误", 'error');
                    }
                },
                error: function () {
        ...
        viewModel.btnSaveClick = function () {
            ...
            var coaDto = (coaDtoList && coaDtoList[0]) ? coaDtoList[0] : emptyCoaDto
