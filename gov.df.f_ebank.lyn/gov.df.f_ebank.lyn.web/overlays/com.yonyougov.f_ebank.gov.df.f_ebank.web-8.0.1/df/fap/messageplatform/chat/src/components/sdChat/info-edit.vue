<template>
  <div class="edit-box" :class="{'bdt-1': !showTextBox}">
    ...
    <div class="sel-box">
      ...
      <span class="icon chat-file" @click="sendFile" title="文件"></span>
    ...
    <div  class="edit-area-box">
      ...
      <div id="chat-content" :disabled="isClose" :contentEditable="!isClose" class="edit-area" @keydown="keydownHandler" @click="getRange"  @keyup="getRange">
        <!--<a onclick="chatOpenModal(billMessageData)" href="#" class="chat-link">${billMessageData.showtext}</a>-->
      </div>
    </div>

    <button class="send-btn" @click="sendMsg" :disabled="isClose">
      发送
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState('sdChat', ['parentNode','groupId', 'iframeName', 'groupUsers', 'isClose']),
      element() {
      ...
      canSendMsg() { // 是否可以发消息
        return !!this.groupUsers.find(user => user.id === this.myUserId) && !this.isClose
      },
    },
    methods: {
      showHistory() {
...
//        let text = $('#chat-content').text().trim()
        ...
        let content = $el.html()
//        console.log(content)
        if(!text && content.indexOf('<img') === -1) return // 没有文字和图片
...
//        msgType = groupId ? 7 : msgType
        ...
        let userId = this.groupId
        // msgType 0 普通消息 1 文件 2 图片消息 5 单据消息  7 群消息
        this.$store.dispatch('sendMsg', {content, groupId, userId, msgType: 7})
        this.$store.commit('SET_ACTION_TYPE', 0)
        $el.html('')
      },
      keydownHandler(e) {
        if(!this.canSendMsg) return
        let {currentDocument: document, currentWindow: window} = this
        if(e.keyCode === 13 && !e.ctrlKey) { // 按enter键, 发送消息
          this.sendMsg()
          e.preventDefault()
        } else if(e.keyCode === 13 && e.ctrlKey) { // 按ctrl+enter, 换行
          let $container = $(document).find('#chat-content')
          $container.html($container.html()+'<div><br></div>')
          var el = $(document).find('#chat-content div:last-child')[0]
      ...
      sendPic() {
        ...
        selectFile((files) => {
          if(files.every(file => /\.(jpg|png|jpeg|gif)$/.test(file.name)))
            this.$store.dispatch('sendFile', {files, groupId: this.groupId, userId: this.groupId})
...
//        selectFile(sendFile)
    ...
    mounted() {
       ...
       * 关闭emoji选择框
       */
      $(window.frames['问题共商'].document).click(() => {
      ...
      this.$nextTick(() => {
        if(this.isClose)
          this.element.html('当前单据已审批结束, 不能发送消息!')
      })
    },
  }
...
//    $('#im-chat').css('display', 'block')
  ...
  function insertTextAtCursor(name, id) {
    let {currentDocument: document, currentWindow: window} = vm
    var sel, img, edit;
