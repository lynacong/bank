...
function ShowDownLoad() {
  ...
  $(document.body).append(
    `<div id="download-niuniu"  style="z-index: 10000; position: fixed; top: 50%; left: 50%; margin-left: -150px; margin-top: -100px; padding: 20px 50px; border: 1px solid #f00; border-radius: 3px; background: #ccc;">
      <span style="font-size: 20px; position: absolute; cursor: pointer; right: 3px; top: -3px;" onclick="javascript: $('#download-niuniu').remove()">&times;</span>
      正在截图, 如果超过5秒无响应, 请点此<a onclick="javascript: $('#download-niuniu').remove()" target="_blank" style="color: #00f;" href="${url.GET_NIUNIU}&${date.getMinutes()}${date.getSeconds()}">安装</a>
    </div>`
  );
}
...
export function StartCapture() {
  // $('#imgshow').hide();
  // $('#imgshow').attr('src', "./image/loading.gif?v=1");
  var picName = '截图' + new Date().format('yyyyMMddhhmmss') + '.jpg'
  var captureRet = captureObj.DoCapture(picName, 0, 0, 0, 0, 0, 0)
  //从返回值来解析显示
...
function OnCaptureFinishedCallback(type, x, y, width, height, info, content, localpath) {
  ...
  switch (type) {
    case 1: {
      ...
      successCallback(content, localpath);
      break;
    }
    case 2: {
      // $('#info').html('您取消了截图');
