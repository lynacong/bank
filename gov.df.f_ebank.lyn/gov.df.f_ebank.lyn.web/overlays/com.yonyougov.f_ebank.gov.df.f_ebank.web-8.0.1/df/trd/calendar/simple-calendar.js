...
var SimpleCalendar = function () {
	...
	//生成日历样式
		...
		value: function create() {
			...
			for (var i = 0; i < 42; i++) {
				days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><div class="data"></div>';
			}
			//添加下拉框数据
	...
	//添加阳历阴历数据
		...
		value: function addData(year, month) {
			...
			for (var i = 0; i < 42; i++) {
				...
				daysElement[i].classList.add(writeyear+'-'+month1+'-'+day);
				var currrent_datas = calendarTwo.solar2lunar(writeyear,writemonth,writeday);
				daysElement[i].querySelector('.day').innerHTML = writeday;
				//判断是否添加阴历
				if (this._options.showLunarCalendar) {
					var IDayCnToIMonthCn = "";
					if(currrent_datas.IDayCn == "初一"){
						IDayCnToIMonthCn = currrent_datas.IMonthCn;
					} else {
						IDayCnToIMonthCn = currrent_datas.IDayCn;
					}
					if(currrent_datas.isTerm){
						daysElement[i].querySelector('.lunar-day').innerHTML = '<div class="sc-festival-jq">' + currrent_datas.Term + '</div>';
					}else{						
						daysElement[i].querySelector('.lunar-day').innerHTML = '<div >' + IDayCnToIMonthCn + '</div>';
					}
				} else {
					if(currrent_datas.isTerm){
						daysElement[i].querySelector('.lunar-day').innerHTML = '<div class="sc-festival-jq">' + currrent_datas.Term + '</div>';
...
//					daysElement[i].querySelector('.lunar-day').innerHTML = '<div class="lunar-day">' + IDayCnToIMonthCn + '</div>';
					daysElement[i].classList.add('item-nolunar');
				}
				daysElement[i].querySelector('.data').innerHTML = '<div class="current-date"'
					+' Animal = "'+ currrent_datas.Animal 
					+'" IDayCn = "'+ currrent_datas.IDayCn 
					+'" IMonthCn = "'+ currrent_datas.IMonthCn 
					+'" astro = "'+ currrent_datas.astro 
					+'" cDay = "'+ currrent_datas.cDay 
					+'" cMonth = "'+ currrent_datas.cMonth 
					+'" cYear = "'+ currrent_datas.cYear 
					+'" gzDay = "'+ currrent_datas.gzDay 
					+'" gzMonth = "'+ currrent_datas.gzMonth 
					+'" gzYear = "'+ currrent_datas.gzYear 
					+'" isToday = "'+ currrent_datas.isToday 
					+'" lDay = "'+ currrent_datas.lDay 
					+'" lMonth = "'+ currrent_datas.lMonth 
					+'" lYear = "'+ currrent_datas.lYear 
					+'" nWeek = "'+ currrent_datas.nWeek 
					+'" isTerm = "'+ currrent_datas.isTerm 
					+'" Term = "'+ currrent_datas.Term 
					+'" ncWeek = "'+ currrent_datas.ncWeek +
					'"></div>';
...
//				}
				...
				if (lunardata && options.showLunarFestival) {
					var currrent_datas = calendarTwo.solar2lunar(year, currentmonth, day);
					if (lunardata[currrent_datas.lMonth + '-' + currrent_datas.lDay]) {
						v.querySelector('.lunar-day').innerHTML = lunardata[currrent_datas.lMonth + '-' + currrent_datas.lDay];
						v.classList.add('sc-festival');
					}
				}
			});
	...
	//刷新主题
		...
		key: 'updateType',
		value: function updateType(year, month) {
			...
			for(var i = 0; i < params.length; i++){
				if(params[i].toLowerCase().indexOf("tokenid=") > -1){
					tokenid = (params[i].split('=')[1]);
				}
			}
			$.ajax({
				url: "/df/Workday/getRestDate.do?tokenid=" + tokenid,
				type: 'GET',
				async: false,
				dataType: 'json',
				data: {"currentdate":data,"ajax":"noCache"},
				success: function (data) {
					var info = data.dateinfo;
					for(var k = 0 ; k < info.length ; k++){
						var date = info[k].china_date;
						var week = info[k].week;
						var type = info[k].date_type;
						var sc_items = $("."+date).find(".current-date");
						if(week < 6 && type == 1 ){
							$("."+date).addClass("sc-vocation");
							sc_items.attr("type",type);
						}else if(week > 5 && type == 0){
							$("."+date).addClass("sc-work");
							sc_items.attr("type",type);
						} else {
							sc_items.attr("type",type);
						}
