...
//千分位金额 youbha
var dealThousandsValue = function dealThousandsValue(amount){
	if(amount){
		amount = parseFloat(amount).toFixed(2);
		if (amount.indexOf(".") == -1) {
			amount = amount.replace(/\d{1,3}(?=(\d{3})+$)/g, function(s) {
				return s + ',';
			});
		} else {
			amount = amount.replace(/(\d)(?=(\d{3})+\.)/g, function(s) {
				return s + ',';
			});
		}
		return amount;
...
/*
 * 获取合计值  wuchr 添加 isSumSelectedRows 是否只是小计选中的数据
 */
var getSumValue = function getSumValue(field, gridCompColumn, gridComp,isSumSelectedRows) {
	var grid_datas = [];
    var sumValue = null;
   if(gridComp.viewModel.gridData){
    	if(gridComp.dataTable.isLazyLoad == 1){
    		if(isSumSelectedRows){
    			if($('#' + gridComp.options.id + '_header_multi_input').hasClass("is-checked")){
    				var current_rows = gridComp.viewModel.gridData.getAllRows();
    			}else{
    				var current_rows = gridComp.viewModel.gridData.getSelectedRows();
    			}
    		}else{
    			var current_rows = gridComp.viewModel.gridData.getAllRows();
    		}
    		for(var i = 0; i < current_rows.length; i++ ){
    			...
    			for(var item in current_rows[i]["data"]) {
    				grid_data["value"][item] = current_rows[i]["data"][item]["value"];
    				grid_data["value"]["$_#_@_id"] = current_rows[i]["rowId"];
    				grid_data["valueIndex"] = i;
    			}
    			grid_datas.push(grid_data);
    		}
    	} else {
    		if(isSumSelectedRows){
    			var current_rows = gridComp.dataTable.getSelectedRows();
        		for(var i = 0; i < current_rows.length; i++ ){
        			var grid_data = {
        				"value": {}
        			};
   ...
   if (gridCompColumn.options.sumCol) {
    ...
    if (sumValue != null && sumValue != undefined && sumValue != 'null' && sumValue != 'undefined') {
    	/*if(typeof gridCompColumn.options.renderType == "function"){
    		sumValue =  
    	}*/
    	/*if(gridCompColumn.options.dataType == 'Float'){
    		sumValue = (sumValue + '').replace(/,/gi,'');
    		sumValue = dealSum(sumValue);
    	}*/
    	return sumValue + '';
    } else {
        return '';
...
var setSortableEnable = function setSortableEnable(sortable,field) {
	for (var i = 0; i < this.gridCompColumnArr.length; i++) {
		if(field){
			if (this.gridCompColumnArr[i].options.field == field) {
				this.gridCompColumnArr[i].options.sortable = sortable;
	        }
		} else {
			this.gridCompColumnArr[i].options.sortable = sortable;
		}
...
var canSortable = function canSortable(e, ele) {
    ...
    console.log(oThis.dataTable.iuap_sort_flag);
    if (sortable) {
    	if(oThis.dataTable.isLazyLoad == 1){
    		var grid_datas = [];
    		var current_rows = oThis.dataTable.iuap_all_data;
    		var k = this.dataSourceObj.options.values[0]["$_#_@_id"];
    		oThis.dataTable.iuap_h = 0;
//	    	var current_rows = this.viewModel.gridData.getAllRows();
			for(var i = 0; i < current_rows.length; i++ ){
				var grid_data = {
					"value": {}
				};
...
//					grid_data["valueIndex"] = i;
//				}	    
					grid_data["value"] = current_rows[i];
					grid_data["value"]["$_#_@_id"] = k++;
					grid_data["valueIndex"] = i;
				grid_datas.push(grid_data);
			}

			this.dataSourceObj.rows = grid_datas;
    	}
        if (e.ctrlKey) {
            // 构建排序信息的数据结构
            var prioArray = [];
            ...
            if ($(".uf-arrow-down").parent().parent().parent()[0] == ele) {
            	oThis.dataTable.iuap_sort_flag = 1;
                //原来为升序，本次为降序
            	...
            	oThis.dataTable.iuap_sort_flag = 0;
                //原来为降序，本次为不排序
                $(".uf-arrow-up").remove();
                if (typeof this.options.onSortFun == 'function') {
        ...
        this.dataTable.sortedData = this.dataSourceObj.rows; //排序后的数据
        oThis.repairContent();
        oThis.afterGridDivsCreate();
        oThis.dataTable.setAllRowsUnSelect();
    }
};
...
var createNoScroll = function createNoScroll(type) {
	if(this.options.multiSelect){
	    var htmlStr = '<div id="' + this.options.id + '_noScroll_' + type + '" class="u-grid-noScroll ' + type + '"><div class="u-grid-noScroll-wrap u-grid-noScroll-wrap-checked ' + type + '" id="' + this.options.id + '_noScroll_' + type + '_wrap">';
...
var createContentLeft = function createContentLeft() {
    ...
    if (this.options.multiSelect) {
        htmlStr += '<div class="u-grid-content-left u-grid-content-left-multi u-grid-content-left1 ' + sumRowClass + '" id="' + this.options.id + '_content_multiSelect" style="width:' + this.multiSelectWidth + 'px;' + hStr + topStr + '">';
        // 遍历生成所有行
    ...
    if (this.options.showNumCol) {
        htmlStr += '<div class="u-grid-content-left u-grid-content-left-num u-grid-content-left2 ' + sumRowClass + '" id="' + this.options.id + '_content_numCol" style="width:' + this.numWidth + 'px;left:' + left + 'px;' + hStr + topStr + '">';
        // 遍历生成所有行
        if (this.dataSourceObj.rows) {
            if (this.options.groupSumRow) {
...
var createContentRows = function createContentRows(createFlag) {
    ...
    if(oThis.dataTable.isLazyLoad == 1){
    	if(oThis.dataTable.is_get_rows_flag == 3 || oThis.dataTable.is_get_rows_flag != oThis.dataTable.iuap_sort_flag){
    		oThis.dataTable.iuap_all_sort_datas = this.dataSourceObj.rows.concat();
    		oThis.dataTable.is_get_rows_flag = oThis.dataTable.iuap_sort_flag;
    	}
    	this.dataSourceObj.rows = this.dataSourceObj.rows.slice(0,100);
    }
    // 遍历生成所有行
    if (this.dataSourceObj.rows) {
...
var createContentOneRowTd = function createContentOneRowTd(row, createFlag) {
    ...
    $.each(gridCompColumnArr, function () {
        ...
        htmlStr += '<td role="rowcell"  ' + tdStyle + classStr + ' realValue="' + v + '" ><div title="'+v.replace(/\</g, '&lt;').replace(/\>/g, '&gt;')+'" class="u-grid-content-td-div ' + cssStr + '" ' + treeStyle + '>' + spanStr + iconStr + '<span>' + v.replace(/\</g, '&lt;').replace(/\>/g, '&gt;') + '</span></div></td>';
    });
    return htmlStr;
};
/*
...
var createContentOneRowTdForIE = function createContentOneRowTdForIE(row, rowObj, createFlag) {
    ...
    $.each(gridCompColumnArr, function () {
        ...
        htmlStr += '<div class="u-grid-content-td-div ' + cssStr + '" ' + treeStyle + ' title="'+v.replace(/\</g, '&lt;').replace(/\>/g, '&gt;')+'">' + spanStr + iconStr + '<span>' + v.replace(/\</g, '&lt;').replace(/\>/g, '&gt;') + '</span></div>';
...
var initGridEventFun = function initGridEventFun() {
    ...
    $('#' + this.options.id + '_header_multi_input').on('click', function (e) {
    	 if(oThis.dataTable.isLazyLoad == 1) {
	    	if(oThis.dataTable.iuap_is_check_all == 0){
	    		oThis.dataTable.iuap_is_check_all = 1;
	    		oThis.dataTable.iuap_is_check_all_still = 0;
	    	} else {
	    		oThis.dataTable.iuap_is_check_all = 0;
	    		oThis.dataTable.iuap_is_check_all_still = 0;
	    	}
    	 }
        if (oThis.hasChecked) {
        	oThis.allRowsSelectedFlag = "true";
            oThis.setAllRowUnSelect();
        } else {
        	oThis.allRowsSelectedFlag = "false";
            oThis.setAllRowSelect();
        }
    });
};
/*计算小计 wuchr*/
var sumSelectRow= function sumSelectRow(oThis){
    if (oThis.options.showSumRow) {
        if (oThis.options.sumRowFixed) {
            if (oThis.options.sumRowFirst) {
                $('#' + oThis.options.id + '_noScroll_begin_table tbody .u-grid-content-sum-row').remove();
                $('#' + oThis.options.id + '_noScroll_fixed_begin_table tbody .u-grid-content-sum-row').remove();
                try {
                    if (oThis.dataSourceObj.rows && oThis.dataSourceObj.rows.length > 0) {
//                        var rowsSelect = oThis.getSelectRows();
                    	var rowsSelect = oThis.dataTable.getSelectedRows();
                        if(rowsSelect.length > 0){
                        	 $('#' + oThis.options.id + '_grid .u-grid-noScroll-left').remove();
                             var type = 'begin';
                             if ((oThis.options.multiSelect || oThis.options.showNumCol) && (type == 'begin' && oThis.options.sumRowFirst && oThis.options.sumRowFixed || type == 'end' && !oThis.options.sumRowFirst && oThis.options.sumRowFixed)) {
                                 var htmlStr = '<div id="' + oThis.options.id + '_noScroll_left" class="u-grid-noScroll-left" style="width:' + oThis.leftW + 'px;height:' + oThis.noScrollHeight + 'px;">';
                                 htmlStr += '</div>';
                                 $('#' + oThis.options.id + '_grid .u-grid-noScroll-wrap')[0].insertAdjacentHTML('afterBegin', htmlStr);
                             }
                            var htmlStr = oThis.createSumRow(false,true);
                            $('#' + oThis.options.id + '_noScroll_begin_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                            var htmlStr = oThis.createSumRow('fixed',true);
                            $('#' + oThis.options.id + '_noScroll_fixed_begin_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                        }else{
                        	 $('#' + oThis.options.id + '_grid .u-grid-noScroll-left').remove();
                             var type = 'begin';
                             if ((oThis.options.multiSelect || oThis.options.showNumCol) && (type == 'begin' && oThis.options.sumRowFirst && oThis.options.sumRowFixed || type == 'end' && !oThis.options.sumRowFirst && oThis.options.sumRowFixed)) {
                            ...
                            var htmlStr = oThis.createSumRow(false,false);
                            $('#' + oThis.options.id + '_noScroll_begin_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                            var htmlStr = oThis.createSumRow('fixed',false);
                            $('#' + oThis.options.id + '_noScroll_fixed_begin_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                        }
                    }
                } catch (e) {//dai wanshan
                    var table = $('#' + oThis.options.id + '_noScroll_begin_table')[0];
                    var fixedTable = $('#' + oThis.options.id + '_noScroll_fixed_begin_table')[0];
                    oThis.createSumRowForIE(table);
                    oThis.createSumRowForIE(fixedTable, 'fixed');
                }
            } else {
                $('#' + oThis.options.id + '_noScroll_end_table tbody .u-grid-content-sum-row').remove();
                $('#' + oThis.options.id + '_noScroll_fixed_end_table tbody .u-grid-content-sum-row').remove();
                try {
                    if (oThis.dataSourceObj.rows && oThis.dataSourceObj.rows.length > 0) {
                    	 var rowsSelect = oThis.getSelectRows();
                         if(rowsSelect.length > 0){
                            var htmlStr = oThis.createSumRow(false,true);
                            $('#' + oThis.options.id + '_noScroll_end_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                            var htmlStr = oThis.createSumRow('fixed',true);
                            $('#' + oThis.options.id + '_noScroll_fixed_end_table tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                         	}
                      }
                } catch (e) {//dai wanshan
                    var table = $('#' + oThis.options.id + '_noScroll_end_table')[0];
                    var fixedTable = $('#' + oThis.options.id + '_noScroll_fixed_end_table')[0];
                    this.createSumRowForIE(table);
                    this.createSumRowForIE(fixedTable, 'fixed');
                }
            }
        } else {
            $('#' + oThis.options.id + '_content_div tbody .u-grid-content-sum-row').remove();
            $('#' + oThis.options.id + '_content_fixed_div tbody .u-grid-content-sum-row').remove();
            try {
                if (oThis.dataSourceObj.rows && oThis.dataSourceObj.rows.length > 0) {
                	var rowsSelect = oThis.getSelectRows();
                    if(rowsSelect.length > 0){
                    	var htmlStr = oThis.createSumRow(false,true);
                    	if (oThis.options.sumRowFirst) {
                            $('#' + oThis.options.id + '_content_div tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                        } else {
                            $('#' + oThis.options.id + '_content_div tbody')[0].insertAdjacentHTML('beforeEnd', htmlStr);
                        }
                    	var htmlStr = oThis.createSumRow('fixed',true);
                        if ($('#' + oThis.options.id + '_content_fixed_div tbody')[0]) {
                            if (oThis.options.sumRowFirst) {
                                $('#' + oThis.options.id + '_content_fixed_div tbody')[0].insertAdjacentHTML('afterBegin', htmlStr);
                            } else {
                                $('#' + oThis.options.id + '_content_fixed_div tbody')[0].insertAdjacentHTML('beforeEnd', htmlStr);
                            }
                        }
                    }else{
                    	var htmlStr = oThis.createSumRow(false,false);
                ...
                var table = $('#' + oThis.options.id + '_content_div table')[0];
                var fixedTable = $('#' + oThis.options.id + '_content_fixed_div table')[0];
                oThis.createSumRowForIE(table);
                oThis.createSumRowForIE(fixedTable, 'fixed');
            }
        }
    }
    if(oThis.options.showSumRow && oThis.options.sumRowFixed && oThis.options.sumRowFirst){
    	//处理合计的千分位
     	 var sumSpan = $('#' + oThis.options.id + '_noScroll_begin_tbody').find("div.u-grid-content-td-div span");
    }else{
    	var sumSpan = $('#' + oThis.options.id + '_content_tbody').find("div.u-grid-content-td-div span");
    }
    for(var j = 0; j < sumSpan.length; j++){
		 if(sumSpan.eq(j).text() != "合计" && sumSpan.eq(j).text() != "小计"){
			 sumSpan.eq(j).text(dealThousandsValue(sumSpan.eq(j).text()));
		 }
	 }  
}
/*
...
var initContentDivEventFun = function initContentDivEventFun() {
    ...
    $('#' + oThis.options.id + '_content .u-grid-content-left').on('click', function (e) {
        ...
        if ($input.length > 0) {
            ...
            if ($input.hasClass('is-checked')) {
                ...
                if(oThis.dataTable.isLazyLoad == 1) {
                	if(oThis.dataTable.iuap_is_check_all == 1){
                		oThis.dataTable.iuap_is_check_all = 0;
                		oThis.dataTable.iuap_is_check_all_still = 1;
                	}
                }
            } else {
            	if(e.shiftKey == 1){
                	if(index > u_start_select_row) {
                		for(var i = u_start_select_row; i <= index; i++ ){
                			oThis.setRowSelect(i);
                		}
                	} else {
                		for(var i = index; i <= u_start_select_row; i++ ){
                			oThis.setRowSelect(i);
                		}
                	}
                } else {
  ...
  //点击序列号事件（一键全屏）youbha
	var global_control_grid=true;
	if(global_control_grid){
		$('#' + oThis.options.id + '_content .u-grid-content-left').on('click', function (e) {
			var $input = $(e.target).closest('.u-grid-content-num');
			if ($input.length > 0) {
				var index = $('#' + oThis.options.id + '_content .u-grid-content-left .u-grid-content-num').index($input);
				var valObject = oThis.getRowByIndex(index).value;
				//把数据详情的展示值与表格展示值同步一致   shengqianga开始
				var arrTableTd = $('#' + oThis.options.id + '_content_tbody').find("tr").eq(index).find("td");
				for(var t = 0; t <arrTableTd.length; t ++){
					if(arrTableTd[t].getAttribute('realvalue') == valObject.status){
						valObject.status = arrTableTd[t].children[0].innerHTML;
					}
				}
				//把数据详情的展示值与表格展示值同步一致   shengqianga结束
				var optionArray = $('#' + oThis.options.id + '_header_thead .u-grid-header-link');
				var columnArray = oThis.options.columns;
				var floatArray=new Array();
				for(var j=0; j<columnArray.length;j++){
					if(columnArray[j].dataType=="Float"){
						floatArray.push(columnArray[j].field);
					}
				}
				var div = "<div class='y-show-Grid'><div class='y-show-header'><h5>数据详情</h5><button type='button' class='btn y-btn y-close' ><i class='fa fa-times'></i></button></div><div class='y-show-detail'></div></div>";
				var has_grid =$(".y-show-Grid")[0];
				if(!has_grid){
					$('#' + oThis.options.id + '_content').append(div);
				}else{
					$(".y-show-Grid").show();
					$(".y-show-detail").empty();
				}
				var grid_height=document.body.clientHeight-120;
				$(".y-show-detail").css("max-height",grid_height);
				var opHtml="",valHtml="";
				for (var i = 0; i < optionArray.length; i++) {
					if(optionArray.eq(i).text() == optionArray.eq(i).parent().parent().attr("field") || optionArray.eq(i).parent().parent().attr("visibleindex")=="-1"){
						opHtml=opHtml;
					}else{
						opHtml += "<li name="+optionArray.eq(i).attr("field")+">"+optionArray.eq(i).text()+"</li>";
						if(valObject[""+optionArray.eq(i).attr("field")+""] !=undefined && valObject[""+optionArray.eq(i).attr("field")+""] !=null){
							if(valObject[""+optionArray.eq(i).attr("field")+""].toString().indexOf("@") != -1 && ( valObject[""+optionArray.eq(i).attr("field")+""] !="" ||valObject[""+optionArray.eq(i).attr("field")+""] !=null)){
								var valArr = valObject[""+optionArray.eq(i).attr("field")+""].split("@");
								for(var q = 0; q< valArr.length; q++){
									if(valArr[q] == undefined || valArr[q] == null){
										valArr[q] = "";
									}
								}
								valHtml +="<li>"+valArr[2]+"&nbsp;"+valArr[1]+"</li>";
							}else
							if("agency_name"==optionArray.eq(i).attr("field") && ( valObject[""+optionArray.eq(i).attr("field")+""] !="" ||valObject[""+optionArray.eq(i).attr("field")+""] !=null)){
								if(!valObject.agency_code){
									valObject.agency_code = "";
								}
								valHtml +="<li>"+valObject.agency_code+"&nbsp;"+valObject[""+optionArray.eq(i).attr("field")+""]+"</li>";
							}else
							if("expeco_name"==optionArray.eq(i).attr("field") && ( valObject[""+optionArray.eq(i).attr("field")+""] !=undefined ||valObject[""+optionArray.eq(i).attr("field")+""] !=null)){
								if(!valObject.expeco_code){
									valObject.expeco_code = "";
								}
								valHtml +="<li>"+valObject.expeco_code+"&nbsp;"+valObject[""+optionArray.eq(i).attr("field")+""]+"</li>";
							}else
							if("expfunc_name"==optionArray.eq(i).attr("field") && ( valObject[""+optionArray.eq(i).attr("field")+""] !=undefined ||valObject[""+optionArray.eq(i).attr("field")+""] !=null)){
								if(!valObject.expfunc_code){
									valObject.expfunc_code = "";
								}
								valHtml +="<li>"+valObject.expfunc_code+"&nbsp;"+valObject[""+optionArray.eq(i).attr("field")+""]+"</li>";
							}else
							if("gov_expeco_name"==optionArray.eq(i).attr("field") && ( valObject[""+optionArray.eq(i).attr("field")+""] !=undefined ||valObject[""+optionArray.eq(i).attr("field")+""] !=null)){
								if(!valObject.gov_expeco_code){
									valObject.gov_expeco_code = "";
								}
								valHtml +="<li>"+valObject.gov_expeco_code+"&nbsp;"+valObject[""+optionArray.eq(i).attr("field")+""]+"</li>";
							}else 
							{
								for(var l = 0; l <= floatArray.length; l++){
									var reg=/%+/;
									if(floatArray[l]==optionArray.eq(i).attr("field")&&valObject[""+optionArray.eq(i).attr("field")+""]!=""&& reg.test(valObject[""+optionArray.eq(i).attr("field")+""])==false){
										valHtml +="<li>"+dealThousandsValue(valObject[""+optionArray.eq(i).attr("field")+""])+"</li>";
										break;
									}
									else if(l < floatArray.length){
										continue;
									}else{
										valHtml +="<li>"+valObject[""+optionArray.eq(i).attr("field")+""]+"</li>";
									}
								}
							}
						}else{
				...
				var html="<ul class='y-show-left' style='float:left;'>"+opHtml+"</ul><ul class='y-show-right' style='float:left;'>"+valHtml+"</ul>";
				$(".y-show-detail").append(html);
				var y_detail_width= $(".y-show-left").width()+$(".y-show-right").width()+20;
				$(".y-show-detail").css("width",y_detail_width);
			}
		...
		$('#' + oThis.options.id + '_noScroll_begin').click(function(){
			if(oThis.options.multiSelect){
				if(oThis.options.is_defined!=null&&oThis.options.is_defined!=undefined&&oThis.options.is_defined==true){
					sumRowClick(oThis.options.id);
				}else{
					
					ip.sumRowClick(oThis.options.id);
				}
			}
		});
	}
    ...
    $('#' + this.options.id + '_content_div').on('scroll', function (e) {
        ...
        oThis.scrollHeight = this.scrollHeight;
        oThis.clientHeight = this.clientHeight;
        ...
        if(oThis.dataTable.isLazyLoad == 1) {
        	var is_scroll_bu = oThis.scrollHeight - oThis.scrollTop - oThis.clientHeight
            if(is_scroll_bu == 0){
            	var now_data = [];
            	var now_data_indexs = [];
            	var iuap_scroll_data = [];
            	
            	if(oThis.dataTable.iuap_sort_flag == 0) {
            		iuap_scroll_data = oThis.dataTable.iuap_result[oThis.dataTable.iuap_h];
            	} else {
            		var d_sort_data = []; 
            		for(var d = 0; d < oThis.dataTable.iuap_all_sort_datas.length; d++){
            			oThis.dataTable.iuap_all_sort_datas[d]["data"] = oThis.dataTable.iuap_all_sort_datas[d]["value"];
            			oThis.dataTable.iuap_all_sort_datas[d]["status"] = "nrm";
            		}
            		var d_all_sort_datas = oThis.dataTable.iuap_all_sort_datas.slice(101).concat();
            		var sort_data_slice = [];
            		for(var k=0,len=d_all_sort_datas.length;k<len;k+=20){
            			sort_data_slice.push(d_all_sort_datas.slice(k,k+20));
            		}
            		iuap_scroll_data = sort_data_slice[oThis.dataTable.iuap_h];
            	}
            	if(iuap_scroll_data && iuap_scroll_data.length > 0){
            		for(var k = 0; k < iuap_scroll_data.length; k++){
            			now_data.push(iuap_scroll_data[k].data);
            			now_data_indexs.push(parseInt(100 + oThis.dataTable.iuap_h * 20 + k));
            		}
            		oThis.viewModel.gridData.addSimpleData(now_data,{unSelect:true});
            		if(oThis.dataTable.iuap_is_check_all == 1){
            			oThis.viewModel.gridData.setAllRowsSelect();
            		}
            		if(oThis.dataTable.iuap_is_check_all_still == 1){
            			oThis.viewModel.gridData.addRowsSelect(now_data_indexs);
            			
            		}
            	}
            	oThis.dataTable.iuap_h++;
            }
        	ip.highLightKeyWord(oThis.dataTable.search_text, "red");//youbha
        }
        if (typeof oThis.options.onScrollFun == 'function') {
        	oThis.options.onScrollFun(oThis);
        }
        if(oThis.options.iuap_callBack){
    		if (typeof oThis.options.iuap_callBack.fun == 'function') {
    			oThis.options.iuap_callBack.fun(oThis.options.iuap_callBack.searchT,oThis.options.iuap_callBack.color,oThis.options.iuap_callBack.area);
            }
        }
    });
    // 数据行相关事件
    ...
    $('#' + this.options.id + '_content_tbody u-grid-checkbox-outline').on('click', function (e) {
        // // 双击处理
        // if (typeof oThis.options.onDblClickFun == 'function') {
        //     oThis.isDblEvent('tbodyClick', oThis.dblClickFun, e, oThis.clickFun, e);
        // } else {
...
var getSelectRows = function getSelectRows() {
	if((this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 1 && this.dataTable.iuap_is_check_all_still== 0) || (this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 0 && this.dataTable.iuap_is_check_all_still== 1)){
		var grid_datas = [];
		var datatable_rows = [];
		if(this.viewModel.gridData){
			//46 you _data
			var datatable_rows = this.viewModel.gridData.getSelectedRows();
			for(var i = 0; i < datatable_rows.length; i++ ){
				var grid_data = {};
				for(var item in datatable_rows[i]["data"]) {
					grid_data[item] = datatable_rows[i]["data"][item]["value"];
					grid_data["$_#_@_id"] = datatable_rows[i]["rowId"];
				}
				grid_datas.push(grid_data);
			}
			this.selectRows = grid_datas;
		}
...
var getSelectRowsIndex = function getSelectRowsIndex() {
	if((this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 1 && this.dataTable.iuap_is_check_all_still == 0) || (this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 0 && this.dataTable.iuap_is_check_all_still == 1)){
		this.selectRowsIndex = this.viewModel.gridData.getSelectedIndices();
	}
...
/*
 * 获取表格排序后的数据  存放在dataTable
 */
var getSortedData = function(type){
	var oThis = this;
	if(type == 'select'){
		if(oThis.dataTable.iuap_sort_flag == 0){
			return  oThis.dataTable.getSimpleData({"type":"select"});
		}else{
			var selectData=oThis.dataTable.getSimpleData({"type":"select"});
			var sortedData=oThis.dataTable.sortedData;
			var lenSorted=sortedData.length;
			var lenSelect=selectData.length;
			var data = [];
			for(var i=0; i<lenSorted;i++){
				for(var j=0;j<lenSelect;j++){
					if(selectData[j].id==sortedData[i].value.id){
						data.push(sortedData[i].value);
						break;
					}	
				}
		...
		if(oThis.dataTable.iuap_sort_flag == 0){
			return oThis.dataTable.iuap_all_data;
		}else{
			var sortedData = oThis.dataTable.sortedData;
			var len = oThis.dataTable.sortedData.length;
			var data = [];
			for(var i=0; i<len;i++){
				data.push(sortedData[i].value);
			}
...
var getAllVisibleColumns = function getAllVisibleColumns() {
    ...
    var obj = $('#' + this.options.id + '_header').find('th');
    if(obj == null){
    	return allVisibleColumns;
    }
    //因为固定列操作区的存在，换种方式判断是否可见，原来的有bug，导致列对不齐，zhangyw。
    for (var i = 0; i < this.gridCompColumnArr.length; i++) {
    	var f1 = this.gridCompColumnArr[i].options.field;
	    for (var j = 0; j < obj.children().length; j++) {
	    	var th = obj.eq(j);
	    	var f2 = th.attr("field");
	    	if(f1 == f2){
...
var getFunObj = {
    ...
    sumSelectRow : sumSelectRow,
    getSortedData: getSortedData //拿到排序后的数据
};
...
var isCheckedHeaderRow = function isCheckedHeaderRow() {
	if((this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 1 && this.dataTable.iuap_is_check_all_still== 0) || (this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 0 && this.dataTable.iuap_is_check_all_still== 1)){
		if (this.selectRowsObj.length !== 0 && this.selectRowsObj.length == this.dataSourceObj.rows.length) {
	        //修改全选标记为false
...
var setRowSelect = function setRowSelect(rowIndex, doms) {
    ...
    if(this.selectRows.length == this.selectRowsObj.length){
    ...
    if((this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 0 && this.dataTable.iuap_is_check_all_still == 1) || (this.dataTable.isLazyLoad == 1 && this.dataTable.iuap_is_check_all == 0 && this.dataTable.iuap_is_check_all_still == 0)){
		if(this.selectRowsObj.length ==  this.dataSourceObj.rows.length){
	        //修改全选标记为false 
	        $('#' + this.options.id + '_header_multi_input').addClass('is-checked');
	        this.dataTable.iuap_is_check_all = 1;
	        this.dataTable.iuap_is_check_all_still = 0;
	        this.hasChecked = true;
		}
    }else{
    ...
    if( this.allRowsSelectedFlag != "false" || this.allRowsSelectedFlag == undefined){
    	this.isCheckedHeaderRow();
  	}
...
//    if( this.allRowsSelectedFlag != "false" ){
...
//    }
      u_start_select_row = rowIndex;
    return true;
};
window.diff = function (obj1,obj2){
 ...
 * 反选一行
 */
var setRowUnselect = function setRowUnselect(rowIndex,allFlag) {
    var oThis = this;
    if (!this.dataSourceObj.rows[rowIndex]) return true;
    //已经选中退出
    ...
    $.each(this.selectRows, function (i) {
        if (diff(this,oThis.dataSourceObj.rows[rowIndex].value)) {
            oThis.selectRows.splice(i, 1);
            oThis.selectRowsObj.splice(i, 1);
            oThis.selectRowsIndex.splice(i, 1);
        }
...
var setAllRowUnSelect = function setAllRowUnSelect() {
	// youbha 取消全选后 将iuap_is_check_all等重置
	if(this.dataTable.isLazyLoad == 1){
		this.dataTable.iuap_is_check_all = 0;
		this.dataTable.iuap_is_check_all_still = 1;
	}
	var oThis = this;
    ...
    for (var i = 0; i < this.dataSourceObj.rows.length; i++) {
        this.setRowUnselect(i,1);
    }
    // end
  /*  if(this.dataSourceObj.rows.length <= 500){
    	if (this.options.multiSelect) {
...
var otherFunObj = {
    ...
    setSortableEnable: setSortableEnable,
...
var setColumnEdit = function setColumnEdit(field) {
	var i;
	var fixColumnArr = [];
	var ColumnArr = [];
	if(field){
		var column = this.getColumnByField(field);
		if(column.options.fixed == true){
			fixColumnArr  = [this.getColumnByField(field)];
		}else{
			ColumnArr = [this.getColumnByField(field)];
		}
	}else{
		 ColumnArr = this.gridCompColumnArr;
		 fixColumnArr  = this.gridCompColumnFixedArr;
	}
    for (i = 0; i < ColumnArr.length; i++) {

        this.editFieldIcon(ColumnArr[i]);
    }

    for (i = 0; i < fixColumnArr.length; i++) {

        this.editFieldIcon(fixColumnArr[i]);
    }     
};
...
/*
 * 创建合计行  wuchr modify【添加复选框选中事件计算小计】
 */
var createSumRow = function createSumRow(createFlag,isSumSelectRows) {
    if (this.options.showSumRow) {
        var oThis = this,
            idStr,
            gridCompColumnArr;
        ...
        $.each(gridCompColumnArr, function (index,value) {
            var f = this.options.field;
            var precision = this.options.precision;
            var dataType = this.options.dataType;
            var sumValue = '';
           /* if(index == '0' && !this.options.sumCol){
            	sumValue = '小计';
            }*/
            if(this.options.sumCol){
            	if(isSumSelectRows){
            		sumValue = oThis.dataSourceObj.getSumValue(f, this, oThis,isSumSelectRows);
            	}else{
            		sumValue = oThis.dataSourceObj.getSumValue(f, this, oThis);
            	}
            ...
            if (!this.options.visible) {
                ...
                if (oThis.options.rowHeight) {
                    tdStyle += 'style="text-align: right;height:' + oThis.options.rowHeight + 'px;line-height:' + oThis.options.rowHeight + 'px;"';
                }
            }
            ...
            htmlStr += '<div class="u-grid-content-td-div" ' + contentStyle + '><span value="' + sumValue + '" id="'+oThis.options.id+f+'">' + sumValue + '</span></div></td>';
        });
        htmlStr += '</tr>';
        return htmlStr;
    }
...
var createSumRowForIE = function createSumRowForIE(table, createFlag) {
    if (this.options.showSumRow) {
        ...
        $.each(gridCompColumnArr, function () {
            ...
            htmlStr += '<span value="' + sumValue + '" id="'+oThis.options.id+ f +'">' + sumValue + '</span></div>';
            newCell.insertAdjacentHTML('afterBegin', htmlStr);
        });
    }
};
        ...
        if (sumCol) {
            ...
            if (sumSpan) {
                if (typeof sumRenderType == 'function') {
                    var sumV = $(sumSpan).attr('value');
                    var obj = {};
                    obj.value = sumV.toString().replace(/,/gi,'');
                    obj.element = sumSpan;
                    obj.gridObj = oThis;
                    obj.gridCompColumn = this;
                    sumRenderType.call(oThis, obj);
