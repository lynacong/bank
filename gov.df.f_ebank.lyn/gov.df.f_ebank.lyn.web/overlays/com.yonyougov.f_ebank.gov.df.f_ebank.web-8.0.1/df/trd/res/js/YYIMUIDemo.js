...
SNSApplication.prototype.login = function(username, password, isToken){
	...
	function parseName(){
			YYIMChat.initSDK("yyzw_6688","yyzw_66");
		}
};
//调用服务器的防腐获取token值
SNSApplication.prototype.getToken = function(username, password){
	...
	var expireTime="";
	 $.ajax({
         type: 'POST',
         url: ctx + "/imaccess/getUserToken.do?tokenid="+tokenid,
         data: {"username":username,"ajax":"nocache"},
         dataType: 'json',
         success: function (result) {
        	 if(result.result=="success")
        		 {
        		 	userToken=result.data.token;
        		 	expireTime=result.data.expiration;
        			YYIMChat.login(username, userToken, expireTime);
        		 }
        	 else
        		 {
...
//			// 直接登录
//			YYIMChat.login("yyg", "612d87fa-594d-4f85-8177-ad4bd531a862", "1473836820285");
//		},
//		error: function(data){
...
var SNSConfig = {
	USER:{
		 ...
		 *  @Type {String}
		 */
		DEFAULT_AVATAR:basePath+"/res/skin/default/icons/noavatar_big.gif",
		DEFAULT_GRAY_AVATAR:basePath+"/res/skin/default/icons/avatar_change.png"
	},
	ROSTER:{
		NAME_FORBIDDEN : ['"',' ','&','\'','/',':','<','>','@'],
		PUB_ACCOUNT_DEFAULT_AVATAR:basePath+"/res/skin/default/icons/public_account_avatar.png",
		MY_TASK_DEFAULT_AVATAR:basePath+"/res/skin/default/icons/my_task_head_icon.png",
		MY_DEVICE_DEFAULT_AVATAR:basePath+"/res/skin/default/icons/my_android.png"
	},
	SYSTEM_ROSTER:{
		DEFAULT_AVATAR:basePath+"/res/skin/default/icons/system_message_avatar.png"
	},
	CHAT_ROOM:{
		DEFAULT_AVATAR:basePath+"/res/skin/default/icons/normal_pic_50x50.png"
	},
	GROUP:{
		GROUP_NONE:"未分组",
...
SNSUser.prototype.pubAccountListHandler = function(list){
	...
	for(var i = 0; i < pubAccountList.length; i++){
		...
		pubAccount.entityType = 'pubaccount';
		...
		if(tab.roster){
			YYIMChat.getVCard({
				...
				success:function(vcard){
					tab.roster.setVCard(new SNSVCard(vcard));
					tab.roster.name = vcard.nickname || tab.roster.id;
					jQuery('.sns_curchat_title').html(tab.roster.name);
					jQuery('#snsim_chat_window_roster_tab_head_'+tab.roster.id).attr('title',tab.roster.name).find('.snsim_username').html(tab.roster.name + '<span class="wbim_icon_vf"></span>');
					tab.roster.photo = vcard.photo;
...
SNSChatWindow.prototype.chatWithAnyRoster = function(arg){
	if(!!arg && arg.id){
        ...
        var roster = user.getRoster(arg.id);
        if(!roster){
        	roster = new SNSRoster(arg.id);
...
SNSChatWindow.prototype.openChatWith = function(roster,name) {
	...
	if(!!temp && temp.entityType == 'chat' && temp.subscription != 'both'){
...
SNSInnerShareTab.fileTemplate = 
	...
	+ '<img width="24" height="28" src="##{{getSNSBasePath()}}/trd/res/skin/default/icons/filetype/##{{type}}.png">'
	+ '</div>'
...
var SNSFileContentRenderFilter = function() {
	...
	this.inTemplate = 
		'<p>'
			+'<img src="##{{getSNSBasePath()}}/trd/res/skin/default/icons/filetype/##{{type}}.png" border="0" class="vm sns_msg_file_icon">'
...
var SNSExpressionData = {
		DEFAULT:{
			...
			priority:0,
			folder:"/trd/res/skin/default/icons/expression/default/",
...
SNSExpressionPanel.prototype._getExpressionHtml = function(){
	...
	for(var i in this.data.DEFAULT.data){
		...
		if(item && item.url){
			...
			var html1=TemplateUtil.genHtml(this.expressionTemplate,[]);
			//$("img").eq(i).attr("src",basePath+"/res/skin/default/icons/expression/default/"+data.DEFAULT.data[i]);
			// console.log("jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj");
			// console.log(item);
		}
//		consloe.log("jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj");
//		consloe.log(item);
	}
...
var SNSFontPanel = function(){
	...
	this.template = 
		'<div id="snsim_font_panel" class="set_font_panel">'
          ...
          +'<input type="text" id="fontColorSelector"/>'
      +'</div>';
	...
	if(jQuery("#snsim_font_panel .sp-replacer").length < 1){
		 $("#fontColorSelector").colorpicker({
	            ...
	            success:function(o,color){
	                SNSIMWindow.getInstance().getChatWindow().getSendBox().getContentDom().css("color", color);
	                SNSMessageStyle.getInstance().color = color;
	                $("#fontColorSelector").val("");
	                $("#fontColorSelector").css("background",color);
	            }
	        });
...
SNSSearchPlugin.prototype._init = function() {
	...
	this.triggerBtn.html = '<a id="snsim_multi_search_trigger_btn" class="multi_search" title="查找"  onclick="setTimeout(function(){jQuery(\'#search_roster\').focus();jQuery(\'#search_roster\').val(\'\')},0);"> </a>';
...
var SNSUserInfoTab = function(){
	...
	this.vcardTpl =  (function (){
		...
		var editBtn = '<a class="edit_vcard_content_btn" style="display:none">修改资料</a><br>'
