		...
		function($, ko, echarts) {
				...
				var year=document.getElementById("set_year" + "-"
					...
					if(year==d.getFullYear()){
						if (status==EBankConstant.WfStatus.TODO_001) {
			        		...
			        		document.getElementById("signandsend").style.display = "";
			        		...
			        		document.getElementById("cancelprint").style.display = "";
			        		document.getElementById("signandsend").style.display = "none";
			        		document.getElementById("sendagain").style.display = "";
			        		document.getElementById("view").style.display = "";
			...
			modalMainGridArea = function(obj) {
				...
				if(document.getElementById("set_year" + "-"  + viewModel.QueryViewId.substring(1, 37)).value==sysDate.getFullYear()){
					if(status=='001'){
						obj.element.innerHTML = '<div class="fun-operate pay-ope" ><a id="'	+ obj.rowIndex + '" onclick="signandsend(this.id)" class="iconmenu icon-sign-sends" title="签章发送"></a></div>';
					}else if(status=='002'){
						obj.element.innerHTML = '<div class="fun-operate pay-ope" ><a id="'	+ obj.rowIndex + '" onclick="sendagain(this.id)" class="iconmenu icon-sends-again" title="重新发送"></a></div>';
					}
				};
			};
			...
			signandsend = function(id){
				...
				optionsParam["vtCode"]=vt_code;
				optionsParam["billnosAndFinanceCode"]=JSON.stringify(billNosAndFinanceCode);
				optionsParam["ajax"]="nocache";
				$.ajax({
					url : EBankConstant.Ctx + "billStampSend"+"/doSignAndSendWithDes.do?tokenid="  + viewModel.tokenid,
					type : "POST",
					data : optionsParam,
			...
			sendagain = function(id){
				...
				optionsParam["ajax"]="nocache";
				$.ajax({
					url : EBankConstant.Ctx + "billStampSend"+"/doAgainSend.do?tokenid=" + viewModel.tokenid,
					type : "POST",
					data : optionsParam,
					success : function(data) {
						if(data.is_success=="0"){
