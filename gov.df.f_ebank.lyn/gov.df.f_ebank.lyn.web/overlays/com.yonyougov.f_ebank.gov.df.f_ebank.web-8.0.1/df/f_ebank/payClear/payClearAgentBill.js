require(
		...
		function($, ko, echarts) {
			...
			var isBH = ip.getUrlParameter("isBH");//是否本行清算数据判断 主要是在确认和本行清算签章界面使用
			...
			createBill=function(id){
				...
				$.ajax({
					url : queryURL+"/doConfirmAgentBill.do",
					type : "POST",
					data :mainOptions,
					success : function(data) {
						if(data.flag=='1'){
							ip.warnJumpMsg("确认成功"+data.result,0,0,true);	
						}else{
			...
			modalMainGridArea = function(obj) {
				...
				if(document.getElementById("set_year" + "-"  + viewModel.QueryViewId.substring(1, 37)).value==sysDate.getFullYear()){
					if(status=='001'){
						obj.element.innerHTML = '<div class="fun-operate pay-ope" ><a id="'	+ obj.rowIndex + '" onclick="createBill(this.id)" class="iconmenu icon-confirms" title="确认"></a><a id="'	+ obj.rowIndex + '" onclick="callBackBill(this.id)" class="iconmenu icon-back-prev" title="退回"></a></div>';
					}
				};
			};
