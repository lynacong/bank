		...
		function($, ko, echarts) {
			...
			modalMainGridArea = function(obj) {
				...
				if(document.getElementById("set_year" + "-"  + viewModel.QueryViewId.substring(1, 37)).value==sysDate.getFullYear()){
					if(status=='001'){
						obj.element.innerHTML = '<div class="fun-operate pay-ope" ><a id="'	+ obj.rowIndex + '" onclick="send(this.id)" class="iconmenu icon-sign-sends" title="签章发送"></a></div>';
					}else if(status=='002'){
			...
			send = function(id){
				...
				$.ajax({
					url :  EBankConstant.Ctx + "billStampSend"+"/doSignAndSendToCzWithDes.do?tokenid="
					+ viewModel.tokenid,
					type : "POST",
					data : optionsParam,
