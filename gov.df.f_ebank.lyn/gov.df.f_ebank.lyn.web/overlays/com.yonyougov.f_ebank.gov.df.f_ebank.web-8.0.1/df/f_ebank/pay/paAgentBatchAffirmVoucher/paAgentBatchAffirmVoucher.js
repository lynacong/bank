			...
			advance=function(id){
				...
				if(billIdsAndFinanceCode.length>1){//判断是否有托收或者空白支票数据
					for (var i = 0; i < billIdsAndFinanceCode.length; i++) {
						var pfName = billIdsAndFinanceCode[i].pf_name;
						if(pfName==null)
							pfName='';
						...
						if(refundMoneyFloat>canRefundMoneyFloat){
							alert("退款金额大于可退金额，可退金额为"+canRefundMoney);;
							return;
						}*/
				...
				if(billIdsAndFinanceCode.length==1){//判断是否有托收或者空白支票数据，如果是的话补录收款信息
						...
						if(index1!=-1||index2!=-1){
							...
							//if(isEditInitPayeeMessage==0)//第一次初始化
							//	initEditPayeeMessageDailog();
							if(index1!=-1){
								pfType=0;
								initEditPayeeMessageForType(pfType);//托收处理
							}else if(index2!=-1){
								pfType=1;
								initEditPayeeMessageForType(pfType);//空白支票处理
							}
							
							//if(payeeMessagetemp.next==0){
								requesting=false;
								return;
							//}
