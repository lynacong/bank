require(
		...
		function($, ko, echarts) {
			...
			buildBillIdsAndFinanceCode = function(id){
				...
				if (id != null && id != undefined && (id + " ").trim().length > 0) {//id不为空
					...
					billIdsAndFinanceCode = viewModel.mainViewModel.gridData.getSimpleData({  //批量选中
						type : 'select',
						fields : [ 'id','bill_no','finance_code','payee_account_bank','payee_account_no','payee_bank_no','pm_code','pm_name','pay_money' ]
					});
				}
			...
			pay=function(id){
				...
				/*if(billIdsAndFinanceCode.length==1){//支付金额大于50000进行指纹验证
				...
				var year=document.getElementById("set_year" + "-"
					...
					if(year==d.getFullYear()){
						if (status==EBankConstant.WfStatus.TODO_001) {
			        		...
			        		//document.getElementById("inputBankNo").style.display = ""
						} else if(status==EBankConstant.WfStatus.AUDITED_002) {
							document.getElementById("pay").style.display = "none";
							document.getElementById("input").style.display = "none";
			        		//document.getElementById("ref").style.display = "none";
			        		//document.getElementById("inputBankNo").style.display = "none"
			        		document.getElementById("view").style.display = "";
