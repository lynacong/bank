require(
		...
		function($, ko, echarts) {
			...
			mainOptions["ch_tx_code"] =ip.getUrlParameter("ch_tx_code");
			mainOptions["pay_type"] =ip.getUrlParameter("pay_type");
			...
			syn=function(){
				...
				mainOptions["set_year"] =document.getElementById("set_year" + "-"
						+ viewModel.QueryViewId.substring(1, 37)).value;
				if(mainOptions["finance_code"]=='000000'||mainOptions["finance_code"]==null||mainOptions["finance_code"]==''){
					ip.warnJumpMsg("请选择具体的财政机构进行同步",0,0,true);
					return;
				}
			...
			//保存时间
			viewModel.saveDateInput=function(){
				var bgn_date=$("#begin_time").val();
				var end_date=$("#end_date").val();
				/*var bgn_date=document.getElementById("bgn_date"
						+ "-" + viewModel.InputViewId.substring(1, 37)).value;
				var end_date=document.getElementById("end_date"
						+ "-" + viewModel.InputViewId.substring(1, 37)).value;*/
				if(bgn_date==null || bgn_date==''){
					ip.warnJumpMsg("请输入开始时间",0,0,true);
					return;
				}
				if(end_date==null || end_date==''){
					ip.warnJumpMsg("请输入结束时间",0,0,true);
					return;
				}
				if(bgn_date>end_date){
					ip.warnJumpMsg("开始时间不能大于结束时间",0,0,true);
					return;
				}
				viewModel.closeDateInput();
				mainOptions["bgn_date"]=bgn_date;
				mainOptions["end_date"]=end_date;
				mainOptions["ajax"]="nocache";
				$.ajax({
					url : EBankConstant.CommonUrl.doSynPayBill+"?tokenid="
						+ viewModel.tokenid,
					type : "POST",
					data :mainOptions,
			...
			//取消
			viewModel.closeDateInput=function(){
				/*document.getElementById("bgn_date"
						+ "-" + viewModel.InputViewId.substring(1, 37)).value='';
					...
					success : function(datas) {
						...
						for ( var n = 0; n < viewModel.viewList.length; n++) {
							...
							if (view.viewtype == EBankConstant.ViewType.VIEWTYPE_LIST) {//列表视图
								...
								if (view.orders == '1'){
									viewModel.InputViewId = view.viewid;
									viewModel.paySearchViewModel = ip
											.initArea(view.viewDetail,
													"input", view.viewid
															.substring(1,
				...
				var year=document.getElementById("set_year" + "-"
					...
					if(year==d.getFullYear()){
						if (status==EBankConstant.WfStatus.TODO_001) {
			        		...
			        		document.getElementById("syn").style.display = "";
