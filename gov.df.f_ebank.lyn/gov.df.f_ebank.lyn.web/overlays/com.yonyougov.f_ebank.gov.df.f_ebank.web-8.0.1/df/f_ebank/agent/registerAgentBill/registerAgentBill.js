require(
		...
		function($, ko, echarts) {
			...
			//撤销登记
			cancelRegisterBill=function(id){
				if(requesting){
					return;
				}
				requesting=true;
			...
			modalMainGridArea = function(obj) {
				...
				var status = $("#pz_status option:selected").val();
		        	if(status=='001'){
						...
						+ obj.rowIndex
						+ '" onclick="registerBill(this.id)" class="iconmenu icon-registers" title="登记"></a></div>';
					}else if(status=='002'){
						obj.element.innerHTML = '<div class="fun-operate pay-ope" ><a id="'
							+ obj.rowIndex
		        	        + '" onclick="cancelRegisterBill(this.id)" class="iconmenu icon-revoke-register" title="撤销登记"></a></div>';
					}
			};
