require(
		...
		function($, ko, echarts) {
			...
			signSend = function(id) {
				...
				$.ajax({
					url : baseURL+"/doSignSendPBToCB.do?tokenid="
					+ viewModel.tokenid,
					type : "POST",
					data : optionsParam,
			...
			reSend = function(id){
				...
				optionsParam["ajax"]="nocache";
				$.ajax({
					url : baseURL+"/doAgainSendPBToCB.do?tokenid="
						+ viewModel.tokenid,
					type : "POST",
					data :optionsParam,
					success : function(data) {
			...
			fs_show = function() {
				...
				if(status=='1'){
					...
					optionsMain["status"] = EBankConstant.WfStatus.UNAUDIT_004;
					optionsSub["status"] = EBankConstant.WfStatus.UNAUDIT_004;
				}
