define(['text!pages/userConfig/userConfig.html',
        'css!pages/userConfig/userConfig.css',
        'jquery','knockout','director','uui', 'tree','md5', "calendar",
...
var init =function(element,param){ 
			...
			treeSetting:{
				...
				callback:{
					onClick:function(e,id,node){
						...
						$.ajax({
							url: "/df/f_ebank/userConfig/usergrid.do?tokenid="+tokenid,
							type: 'GET',
							dataType: 'json',
							data: {"chr_id":chr_id,"ajax":"noCache"},
							success: function (data) {
							createGrid(data.dataDetail);
						/*	writeGrid = $('#write-grid').DataTable({
 									destroy: true,
					 			    searching: false,
					 		        paging: false,
					 		        bSort: false,
 							        ...
 							        columns: [
 							            { "data": "user_code","title":"用户编码"},
 							            { "data": "user_name","title":"用户名称"},
 							            { "data": "nickname","title":"用户昵称","visible":false},
							            { "data": "belong_type","title":"所属类型"},
							            { "data": "orgname","title":"用户所属"},
 							            { "data": "datarule","title":"数据权限","visible":false},
 							            { "data": "security_level","title":"安全级别","visible":false},
							            { "data": "headship_code","title":"职务","visible":false},
							            { "data": "gender","title":"性别","visible":false},
 							            { "data": "birthday","title":"出生日期","visible":false},
 							            { "data": "address","title":"家庭地址","visible":false},
							            { "data": "telephone","title":"固定电话","visible":false},
							            { "data": "mobile","title":"移动电话","visible":false},
 							            { "data": "imsi","title":"手机IMSI","visible":false},
 							            { "data": "imei","title":"手机IMEI","visible":false},
							            { "data": "is_black_list","title":"黑名单","visible":false},
							            { "data": "enabled","title":"启用标识"},
 							            { "data": "use_lock","title":"手工锁定","visible":false},
 							            { "data": "is_ever_locked","title":"是否锁定","visible":false},
							            { "data": "auto_lock","title":"自动锁定","visible":false},
							            { "data": "role_info","title":"角色信息"},
							            { "data": "user_id","title":"id","visible":false}
 							        ],
 							       "columnDefs" : [{
 					    				"targets" : [2,5,6,7,8,10,11,12,13,14,15,17,18,19,21],
	...
	menuViewModel.getInitData = function () {
		...
		getPara();
		$.ajax({
			url: "/df/f_ebank/userConfig/usertree.do?tokenid=" + tokenid,
			type: 'GET',
			dataType: 'json',
			data: {"ajax":"noCache"},
			success: function (data) {
	...
	$("body").on('click', '#write-grid tbody tr', function () {
	...
	menuViewModel.editbtnSubmit = function () {
		...
		var selRows = $('#write-grid').DataTable().rows('.selected');
		clearAll();
		isNew = "0";
		if(selRows.indexes().length == 0){
			ip.ipInfoJump("请选择需要修改的用户","error");
			return;
		}
		if(selRows.length > 1){
			ip.ipInfoJump("请不要选择一个以上用户","error");
			return;
		}

		user_id = selRows.data()[0].user_id;
		var user_code = selRows.data()[0].user_code;
		var user_name = selRows.data()[0].user_name;
		var nickname = selRows.data()[0].nickname;
		var belong_type = selRows.data()[0].belong_type;
		var belong_org = selRows.data()[0].belong_org;
		var security_level = selRows.data()[0].security_level;
		var gender = selRows.data()[0].gender;
		var birthday = selRows.data()[0].birthday;
		var org_type = selRows.data()[0].org_type;
		var mobile = selRows.data()[0].mobile;
		var imsi = selRows.data()[0].imsi;
		var imei = selRows.data()[0].imei;
		var enabled = selRows.data()[0].enabled;
		var role_info = selRows.data()[0].role_info;
		passwordold = selRows.data()[0].password;
		var identity_card = selRows.data()[0].identity_card;
		if(gender == "1"){
		...
		$.ajax({
			url: "/df/f_ebank/userConfig/initMessage.do?tokenid=" + tokenid,
			type: 'GET',
			dataType: 'json',
			...
			success: function (data) {
				...
				if(selectRoles != null && selectRoles != ""){
					for(var i=0; i<selectRoles.length; i++){
						...
						allRoletreeObj.expandNode(search_nodes[0], true, true, true);
					}
				}
				//基本设置修改初始化
	...
	menuViewModel.delbtnSubmit = function () {
		...
		$("#sid1").on("click",function(){
			...
			for(var k = 0 ; k < selRows.length ; k++){
				var user_id = selRows.data()[k].user_id;
				allid = user_id + "@"+allid;
			}

			$.ajax({
				url: "/df/f_ebank/userConfig/deleteUser.do?tokenid="+tokenid,
				type: "POST",
				data: {
					"ajax": "1",
					"user_id":allid
	...
	menuViewModel.lockbtnSubmit = function () {
		...
		var selRows = $('#write-grid').DataTable().rows('.selected'); 
		if(selRows.indexes().length == 0){
			ip.ipInfoJump("请选择要锁定的数据","error");
			return;
		}
		var allid = "";
		var errorMessage = "";
		for(var k = 0 ; k < selRows.length ; k++){
			if(selRows.data()[k].is_ever_locked == "0"){
				var user_code = selRows.data()[k].user_code;
				var codeEnd = user_code.substring(6,9);
				if(user_code.length == 9 && codeEnd == "999"){
					ip.ipInfoJump("不能锁定'xxxxxx999'的用户","error");
				}else{
					var user_id = selRows.data()[k].user_id;
					allid = user_id + "@"+allid;
				}
				...
				var user_code = selRows.data()[k].user_code;
				var user_name = selRows.data()[k].user_name;
		...
		if(errorMessage != "" && errorMessage != null){
			...
			ip.ipInfoJump("选择的"+errstrnew + "已经被锁定","error");
		}
		if(allid == "" || allid == null){
			return;
		}
		$.ajax({
			url: "/df/f_ebank/userConfig/locked.do?tokenid="+tokenid,
			type: "POST",
			data: {
				"ajax": "1",
				"user_id":allid
			...
			success: function(data){
				if(data.flag == "1"){
					ip.ipInfoJump("成功锁定"+data.num +"条用户","error");
					refreshgrid();
				}
			}
		});
	...
	menuViewModel.unlockbtnSubmit = function () {
		var selRows = $('#write-grid').DataTable().rows('.selected'); 
		if(selRows.indexes().length == 0){
			ip.ipInfoJump("请选择要解锁的数据","error");
			return;
		}
		var allid = "";
		var errorMessage = "";
		for(var k = 0 ; k < selRows.length ; k++){
			if(selRows.data()[k].is_ever_locked == "1"){
		...
		if(errorMessage != "" && errorMessage != null){
			...
			var errstr = errorMessage.substring(0,len-1);
			var errstrnew = errstr.replaceAll("@","和");
			ip.ipInfoJump("选择的"+errstrnew + "未被锁定","error");
		}

		$.ajax({
			url: "/df/f_ebank/userConfig/unlocked.do?tokenid="+tokenid,
			type: "POST",
			data: {
				"ajax": "1",
				"user_id":allid
			...
			success: function(data){
				if(data.flag == "1"){
					ip.ipInfoJump("成功解锁"+data.num +"条用户","error");
					refreshgrid();
				}
			}
		});
...
//	添加角色弹窗
	menuViewModel.showAddRolesModal = function(){
		var tokenid = ip.getTokenId();
		$.ajax({
			url: "/df/f_ebank/userConfig/findrole.do?tokenid="+tokenid,
			type: "GET",
			data: {
				"ajax": "1"
			},
...
//	显示每个权限组的详细信息
	...
	menuViewModel.userSave = function(){
		...
		if(usernewcode != usercode){
			ip.ipInfoJump("用户编码只允许汉字、字母、数字、中横杠-、下横杠_!","error");
			return;
		}
		...
		if(userorgtype != "001"){
			if(beloneorg == null || $.trim(beloneorg) == ""){
				...
				dataoptions["belong_org_name"]=bl_org[1];
				dataoptions["belong_org_code"]=bl_org[2];
			}
		}
		...
		if(org_id == "" && userorgtype != "001"&& userorgtype != "119"){
			ip.ipInfoJump("机构权限不可为空","error");
			return;
		}
		...
		var tokenid = ip.getTokenId();
		$.ajax({
			url: "/df/f_ebank/userConfig/addUser.do?tokenid="+tokenid,
			type: "POST",
			data: dataoptions,
			success: function(data){
				var flag = data.flag;
...
/*		var b = document.getElementById("security_level");
	...
	menuViewModel.rightSure = function() {
		...
		for(var i=0 ; i < eleSize ; i++)
		{
			...
			if(isChecked == 0)
			{
				...
				if(obj == null)
				{
					ip.ipInfoJump("选择部分权限必须至少勾选一个要素！","error");
					return;
				}
				right_type = 1 ;
				for(var j=0 ;j< eleObj.length ;j++)
	...
	orgTypeChange = function(obj){
		...
		$.ajax({
			url: "/df/f_ebank/userConfig/findEleCodeByOrgtypeCode.do?tokenid="+tokenid,
			type: 'GET',
			dataType: 'json',
...
//	动态生成树的方法----开始
	 treeChoice=function(id,data,index) {
...
//			日期
	 ...
	 baseValidate=function(str,baseThis,str1){
	 ...
	 blurValidate=function(str,baseThis,msg){
		baseThis.each(function(){
			$(this).blur(function(){
				var _thisValue = $(this).val();
	...
	refresh=function(){
		menuViewModel.getInitData();
		$('#write-grid').dataTable().fnClearTable();
